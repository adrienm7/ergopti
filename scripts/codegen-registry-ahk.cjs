...
function buildAhkSource() {
	...
	lines.push('\t\tlocal group       := opts.Has("group")        ? opts["group"]        : REGISTRY_DEFAULT_GROUP');
	...
	lines.push('\t\t\t\tLoggerWarn("registry", "Add: duplicate trigger \'{1}\' in group \'{2}\' — skipped.", trigger, group)');
	...
	lines.push("\t}");
	lines.push("");
	lines.push("\t; Sorts a single bucket in-place: longest trigger first, then group_order, then seq.");
