...
An ADR is a short document that captures a significant architectural decision: the context that led to it, the decision itself, and its consequences. ADRs form a permanent, searchable log of _why_ the codebase is shaped the way it is — not just _what_ it does.
