// scripts/codegen-prompt-builder-ahk.cjs
...
function buildAhkSource() {
	...
	lines.push(`\t\tlocal parts := StrSplit(Trim(buffer), [${ASP}, ${AQ}\`t${AQ}, ${AQ}\`n${AQ}, ${AQ}\`r${AQ}])`);
	...
	lines.push("\t\t}");
	lines.push("\t\treturn Max(PB_MIN_MAX_TOKENS, maxWords * PB_WORDS_TO_TOKENS_RATIO + PB_TOKEN_BUDGET_OVERHEAD)");
	lines.push("\t}");
	lines.push("");
	lines.push("");
	lines.push("");
	...
	lines.push("\t\t\tt := Min(PB_TEMP_DIVERSITY_CAP, t + PB_TEMP_INCREMENT_PER_PRED * (numPredictions - 1))");
	lines.push("\t\t}");
	lines.push("");
	...
	lines.push("\t\t}");
	lines.push("\t\tlocal charLimit := Max(PB_CONTEXT_MIN_CHARS, maxWords * PB_CONTEXT_CHARS_PER_WORD)");
	...
	lines.push(`\t\tlocal maxWords       := config.Has(${AQ}max_words${AQ})       ? config[${AQ}max_words${AQ}]       : 0`);
	lines.push(`\t\tlocal minWords       := config.Has(${AQ}min_words${AQ})       ? config[${AQ}min_words${AQ}]       : 1`);
	lines.push(`\t\tlocal numPredictions := config.Has(${AQ}num_predictions${AQ}) ? config[${AQ}num_predictions${AQ}] : 1`);
	lines.push(`\t\tlocal temperature    := config.Has(${AQ}temperature${AQ})     ? config[${AQ}temperature${AQ}]     : 0.1`);
	lines.push(`\t\tlocal autoRaise      := config.Has(${AQ}auto_raise_temp${AQ}) ? config[${AQ}auto_raise_temp${AQ}] : false`);
	lines.push(`\t\tlocal language       := config.Has(${AQ}language${AQ})        ? config[${AQ}language${AQ}]        : ${AQ}fr${AQ}`);
	lines.push("");
	...
	lines.push(`\t\t\t${AQ}temperature${AQ},      this._ComputeTemperature(temperature, numPredictions, autoRaise),`);
