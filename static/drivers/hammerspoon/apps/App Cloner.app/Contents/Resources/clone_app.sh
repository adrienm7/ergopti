...
def _sync_space_assignment():
    ...
    try:
        ...
        if not os.path.exists(src_id_path):
            return
        with open(src_id_path) as fh:
            src_bid = fh.read().strip()
        if not src_bid:
            return
        ...
        assignments = dock.get("workspaces-application-assignments", {}) or {}

        # Source app's assignment in dock.plist takes precedence — it covers both
        # explicit "Assign to this desktop" (UUID value) and "All Desktops"
        # (sentinel string). If the source app has no entry, scan the spaces
        # plist to follow the window's actual placement set via Mission Control.
        target = assignments.get(src_bid)
        if not target:
            try:
                ...
                if result.returncode == 0:
                    ...
                    for display in data.get("ManagedDisplaySpaces", []):
                        for space in display.get("Spaces", []):
                            if src_bid in space.get("apps", []):
                                target = space.get("uuid", "")
                                break
                        if target:
                            break
            except Exception:
        ...
        current = assignments.get(MY_BUNDLE_ID)
        changed = False
        if target and current != target:
            assignments[MY_BUNDLE_ID] = target
            dock["workspaces-application-assignments"] = assignments
            changed = True
        elif not target and MY_BUNDLE_ID in assignments:
            del assignments[MY_BUNDLE_ID]
            dock["workspaces-application-assignments"] = assignments
            changed = True

        if changed:
            with open(dock_path, "wb") as fh:
                plistlib.dump(dock, fh)
            # Dock caches the plist in memory — restart it to pick up the
