...
def extract_hotstring_from_line(
    ...
    """
    # Patterns covering both simple and complex argument cases
    hotstring_patterns = [
        # Simple: CreateHotstring(..., "trigger", "output")
        r'CreateHotstring\([^,]+,\s*"([^"]+)"\s*,\s*"([^"]+)"',
        r'CreateCaseSensitiveHotstrings\([^,]+,\s*"([^"]+)"\s*,\s*"([^"]+)"',
        # Complex: CreateHotstring(..., "trigger" . ScriptInformation["MagicKey"], "output")
        r'CreateHotstring\([^,]+,\s*"([^"]+)"\s*\.\s*ScriptInformation\["MagicKey"\],\s*"([^"]+)"',
        r'CreateCaseSensitiveHotstrings\([^,]+,\s*"([^"]+)"\s*\.\s*ScriptInformation\["MagicKey"\],\s*"([^"]+)"',
        # SuffixesA pattern: CreateCaseSensitiveHotstrings("*?", "trigger", "output", Map(...))
        r'CreateCaseSensitiveHotstrings\(\s*"[^"]*",\s*"([^"]+)"\s*,\s*"([^"]+)"\s*,\s*Map\(',
    ]
    ...
    hotstring_patterns = [
        ...
        r'CreateCaseSensitiveHotstrings\([^,]+,\s*"([^"]+)"\s*,\s*"([^"]+)"',
        # Complex: CreateHotstring(..., "trigger" . ScriptInformation["MagicKey"], "output")
        # Capture the trigger part + the magic key (★) as one group
        r'CreateHotstring\([^,]+,\s*"([^"]+)"\s*\.\s*ScriptInformation\["MagicKey"\],\s*"([^"]+)"',
        r'CreateCaseSensitiveHotstrings\([^,]+,\s*"([^"]+)"\s*\.\s*ScriptInformation\["MagicKey"\],\s*"([^"]+)"',
        # SuffixesA pattern: CreateCaseSensitiveHotstrings("*?", "trigger", "output", Map(...))
        r'CreateCaseSensitiveHotstrings\(\s*"[^"]*",\s*"([^"]+)"\s*,\s*"([^"]+)"\s*,\s*Map\(',
    ...
    for pattern in hotstring_patterns:
        ...
        if match:
            ...
            # If the line contains ScriptInformation["MagicKey"], replace the variable with ★
            if 'ScriptInformation["MagicKey"]' in line:
                # Replace the MagicKey variable with ★ in the original line to get the full trigger
                trigger_with_magic = re.sub(
                    r'"([^"]+)"\s*\.\s*ScriptInformation\["MagicKey"\]',
                    r'"\1★"',
                    line,
                )
                # Extract the full trigger including the ★
                magic_match = re.search(r'"([^"]+★)"', trigger_with_magic)
                if magic_match:
                    trigger = magic_match.group(1)
...
def convert_to_toml(
    ...
    for section_name, entries in hotstrings.items():
        ...
        for trigger, output in entries:
            # Escape only quotes in trigger, escape backslashes and quotes in output
