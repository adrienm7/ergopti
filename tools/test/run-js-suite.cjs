...
const CHECKS = [
	...
	{ name: 'every registered menu row matches its declared type (handler slot vs provider slot)', cmd: 'node', args: ['tools/test/test-menu-provider-kind-matches-type.cjs'], repro: 'node tools/test/test-menu-provider-kind-matches-type.cjs' },
	...
	{ name: 'every top-level menu title reads ONE shared key on every driver that has it', cmd: 'node', args: ['tools/test/test-menu-titles-single-key.cjs'], repro: 'node tools/test/test-menu-titles-single-key.cjs' },
	{ name: 'every declared menu is answered by every driver that shows it (per-category coverage)', cmd: 'node', args: ['tools/test/test-menu-category-coverage.cjs'], repro: 'node tools/test/test-menu-category-coverage.cjs' },
	{ name: 'one hold picker: the same options in the same order on every driver, from one shared table', cmd: 'node', args: ['tools/test/test-tap-hold-hold-options-parity.cjs'], repro: 'node tools/test/test-tap-hold-hold-options-parity.cjs' },
	{ name: 'the hotstring category submenus agree across the three drivers (same controls, same order)', cmd: 'node', args: ['tools/test/test-hotstring-category-submenu-order.cjs'], repro: 'node tools/test/test-hotstring-category-submenu-order.cjs' },
	{ name: 'hotstring language packs are complete data and every section ships disabled', cmd: 'node', args: ['tools/test/test-hotstring-language-packs.cjs'], repro: 'node tools/test/test-hotstring-language-packs.cjs' },
	...
	{ name: 'every declared action has a handler in its driver (I4 bijection — ahk 0, hs 0, linux 0)', cmd: 'node', args: ['tools/test/test-action-registry-bijection.cjs'], repro: 'node tools/test/test-action-registry-bijection.cjs' },
	...
	{ name: 'every app window keeps the shared frame on macOS, Windows and Linux (no edgeless window)', cmd: 'node', args: ['tools/test/test-app-windows-keep-their-frame.cjs'], repro: 'node tools/test/test-app-windows-keep-their-frame.cjs' },
	...
	{ name: 'Lua test fixtures stay out of the repository (no working-directory fallback)', cmd: 'node', args: ['tools/test/test-lua-fixtures-stay-out-of-the-repo.cjs'], repro: 'node tools/test/test-lua-fixtures-stay-out-of-the-repo.cjs' },
	...
	{ name: 'API test-request single source (probe text lives in api_providers.json only)', cmd: 'node', args: ['tools/test/test-llm-test-request-single-source.cjs'], repro: 'node tools/test/test-llm-test-request-single-source.cjs' },
	{ name: 'API model-extras single source (per-model body fields live in api_providers.json only)', cmd: 'node', args: ['tools/test/test-llm-model-extras-single-source.cjs'], repro: 'node tools/test/test-llm-model-extras-single-source.cjs' },
	...
	{ name: 'macOS app ships the keyboard layout bundle its menu resolves (no "No Ergopti bundle found")', cmd: 'node', args: ['tools/test/test-macos-keyboard-layout-bundle.cjs'], repro: 'npm run test:macos-keyboard-layout-bundle' },
	...
	{ name: 'the preview bubble is rendered from the shared masking corpus (ratchet: which drivers carry it)', cmd: 'node', args: ['tools/test/test-preview-masking-cross-driver.cjs'], repro: 'node tools/test/test-preview-masking-cross-driver.cjs' },
	...
	{ name: 'every log/persist sink that can carry a hotstring trigger is redacted or judged (derived list, not a written one)', cmd: 'node', args: ['tools/test/test-personal-info-log-sinks-are-judged.cjs'], repro: 'node tools/test/test-personal-info-log-sinks-are-judged.cjs' },
	...
	{ name: 'drift guard detects targeted output changes and preserves exact edits', cmd: 'node', args: ['tools/test/test-drift-guard-covers-every-output.cjs'], repro: 'node tools/test/test-drift-guard-covers-every-output.cjs' },
	{ name: 'drift coverage rejects invalid receipts and restores damaged targets', cmd: 'node', args: ['tools/test/test-drift-guard-coverage-oracle.cjs'], repro: 'node tools/test/test-drift-guard-coverage-oracle.cjs' },
	{ name: 'drift restoration preserves healthy siblings after filesystem refusals', cmd: 'node', args: ['tools/test/test-drift-restore-refusal.cjs'], repro: 'node tools/test/test-drift-restore-refusal.cjs' },
	...
	{ name: 'feature manifest parity (Windows, macOS, and Linux projections agree)', cmd: 'node', args: ['tools/test/test-manifest-parity.cjs'], repro: 'node tools/test/test-manifest-parity.cjs' },
	{ name: 'Linux feature parity status (326 rows with explicit ownership and proof tiers)', cmd: 'node', args: ['tools/test/test-linux-feature-parity.cjs'], repro: 'npm run test:linux-feature-parity' },
	...
	{ name: 'App Cloner launchers isolate runtime arguments', cmd: 'node', args: ['tools/test/test-app-cloner-run-isolation.cjs'], repro: 'npm run test:app-cloner-run-isolation' },
	{ name: 'App Cloner logging stays isolated from generated launchers', cmd: 'node', args: ['tools/test/test-app-cloner-log-isolation.cjs'], repro: 'npm run test:app-cloner-log-isolation' },
	{ name: 'App Cloner generated literals survive hostile values', cmd: 'node', args: ['tools/test/test-app-cloner-generated-literals.cjs'], repro: 'npm run test:app-cloner-generated-literals' },
	{ name: 'App Cloner Dock URLs remain canonical', cmd: 'node', args: ['tools/test/test-app-cloner-dock-url.cjs'], repro: 'npm run test:app-cloner-dock-url' },
	{ name: 'Ollama bootstrap bounds and authenticates network acquisition', cmd: 'node', args: ['tools/test/test-ollama-bootstrap-network-hardening.cjs'], repro: 'npm run test:ollama-bootstrap-network-hardening' },
	{ name: 'Ollama server command preserves exact process ownership', cmd: 'node', args: ['tools/test/test-ollama-server-command.cjs'], repro: 'npm run test:ollama-server-command' },
	...
	{ name: 'WPM readout constants single source (every canon key read by a driver, none restated)', cmd: 'node', args: ['tools/test/test-wpm-constants-single-source.cjs'], repro: 'node tools/test/test-wpm-constants-single-source.cjs' },
	{ name: 'WPM strip darkening cross-driver drift (Lua model and AHK round-half-up on the canon factor; golden vectors)', cmd: 'node', args: ['tools/test/test-wpm-color-normalisation-single-source.cjs'], repro: 'node tools/test/test-wpm-color-normalisation-single-source.cjs' },
