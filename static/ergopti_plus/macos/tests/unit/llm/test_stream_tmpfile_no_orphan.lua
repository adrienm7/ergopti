...
local function check_file(rel_path, suffix)
	...
	helpers.assert_true(has_base_var, rel_path .. ": os.tmpname() call must still be present")
	...
	helpers.assert_true(
		remove_base,
		rel_path .. ": os.remove(_tmp_base) must appear after os.tmpname() to remove the orphan base file (" .. suffix .. ")"
	)
	...
	helpers.assert_true(
		uses_suffix,
		rel_path .. ": suffixed tmp path (" .. suffix .. ") must still be used for the payload"
	)
end
