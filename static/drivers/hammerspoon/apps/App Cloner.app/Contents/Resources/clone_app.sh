...
#
# Architecture: this script stays alive as the main process of the stub bundle.
# The Dock tile therefore runs under our CFBundleIdentifier (set via
# __CFBundleIdentifier), not under Edge's. Edge is a child process that we
# wait on. When the user quits Edge the launcher exits too, keeping the Dock
# tile lifecycle in sync.
#
# Focus detection: we write our own PID to a lock file at startup so that a
# second click on the Dock tile finds us, activates the Edge window via
# osascript, and exits — no duplicate window.

LOCK_FILE="${PWA_PROFILE_DIR}/appcloner.lock"
LOG_FILE="/tmp/appcloner_pwa_${UNIQUE_ID}.log"

# If a lock file exists check whether that launcher process is still alive.
if [[ -f "\$LOCK_FILE" ]]; then
	OLD_PID="\$(cat "\$LOCK_FILE" 2>/dev/null)"
	if [[ -n "\$OLD_PID" ]] && kill -0 "\$OLD_PID" 2>/dev/null; then
		# Launcher still alive → PWA is running. Find the Edge child that owns
		# this profile and bring it to the front.
		EDGE_PID="\$(pgrep -P "\$OLD_PID" 2>/dev/null | head -1)"
		TARGET_PID="\${EDGE_PID:-\$OLD_PID}"
		osascript -e "tell application \"System Events\" to set frontmost of (first process whose unix id is \$TARGET_PID) to true" 2>/dev/null || true
		exit 0
	fi
	# Stale lock — launcher died without cleanup. Remove it and start fresh.
	rm -f "\$LOCK_FILE"
fi

# Write our PID so a second Dock click can find and focus us.
echo \$\$ > "\$LOCK_FILE"

# Trap EXIT to always clean up the lock file, even on crash or SIGTERM.
trap 'rm -f "\$LOCK_FILE"' EXIT INT TERM

# On first launch, switch to the target Space so Edge opens there naturally.
# The Space UUID was written by the clone-creation script; we consume it once
# and delete the hint so subsequent launches let Edge remember its own position.
SPACE_HINT="${PWA_PROFILE_DIR}/target_space_uuid"
if [[ -f "\$SPACE_HINT" ]]; then
	TARGET_UUID="\$(cat "\$SPACE_HINT")"
	rm -f "\$SPACE_HINT"
	# Use swift to call the private CGS API that switches the active Space.
	# This is the only reliable way to change Space without simulating keyboard
	# shortcuts (which would break if the user has remapped Mission Control keys).
	swift - "\$TARGET_UUID" <<'SWIFTEOF' 2>/dev/null || true
import CoreGraphics
import Foundation
...
let match = spaces.first { ($0["uuid"] as? String) == targetUUID }
guard let spaceID = match?["ManagedSpaceID"] as? Int else { exit(0) }
CGSShowSpaces(cid, [spaceID] as CFArray)
SWIFTEOF
fi

# Launch Edge as a child (not exec) so this script stays alive and the Dock
# tile stays associated with our bundle-id rather than Edge's.
# --disable-features flags suppress the "Open in desktop app" infobar that
# Chromium shows when it detects a protocol handler for Teams / Outlook.
"${PWA_BROWSER_EXE}" \\
	--app="${OPEN_ARG}" \\
...
# Wait for Edge to exit. When the user quits Edge this script exits too,
# which removes the lock file (via the trap above) and collapses the Dock tile.
wait \$EDGE_PID
PWAEOF
	chmod +x "$LAUNCHER"
