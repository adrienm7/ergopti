...
local CONFIG = {
	...
	-- Compact mode (no graph): colored pill with big number + small unit label.
	compact_number_size   = 28,  -- large WPM digit
	compact_unit_size     = 11,  -- small "mpm" label below
	compact_padding_x     = 14,
	compact_padding_top   = 8,
	compact_gap           = 4,   -- gap between number and unit rows
	compact_padding_bottom = 8,

	-- Graph mode: same style as before.
...
local function update_widget()
	...
	if display_wpm > 0 or tooltip_visible or (active_source ~= "none") then
		...
		if _show_graph then
			...
			-- Compact: size to fit two rows (number + unit label).
			local num_h  = CONFIG.compact_number_size + 6
			local unit_h = CONFIG.compact_unit_size + 4
			local num_measure = hs.drawing.getTextDrawingSize(wpm_number_str, { size = CONFIG.compact_number_size, font = ".AppleSystemUIFont" })
			local num_w = (num_measure and num_measure.w) or math.floor(#wpm_number_str * CONFIG.compact_number_size * 0.65)
			canvas_width  = math.max(60, math.floor(num_w + CONFIG.compact_padding_x * 2))
			canvas_height = CONFIG.compact_padding_top + num_h + CONFIG.compact_gap + unit_h + CONFIG.compact_padding_bottom
		...
		if _show_graph then
			...
			-- ── Compact mode: colored pill — big number + small unit label ─────
			...
			local bg_color = { hex = source_color.hex, alpha = 0.85 }

			table.insert(elements, { type = "rectangle", action = "fill", fillColor = bg_color, roundedRectRadii = { xRadius = bg_radius, yRadius = bg_radius } })

			local num_h   = CONFIG.compact_number_size + 6
			local unit_h  = CONFIG.compact_unit_size + 4
			local num_y   = CONFIG.compact_padding_top
			local unit_y  = num_y + num_h + CONFIG.compact_gap
			...
			table.insert(elements, {
				...
				frame = { x = 0, y = num_y, w = canvas_width, h = num_h },
			})
			table.insert(elements, {
				...
				textAlignment = "center",
				frame = { x = 0, y = unit_y, w = canvas_width, h = unit_h },
			})
		end
