...
- **Project architecture & decisions**
  ...
  - [project-hs-perf-profilers-and-case-conform](#project-hs-perf-profilers-and-case-conform) — macOS boot/hot-path profilers (via timer_scheduler adapter), the case-conform registration fast path (lowercase-only + fire-time conform, Unicode tail buckets), the menubar dirty-cache, and the escape-trap-on-first-show gotcha (Escape priority over Raycast — do not move to init)
  - [project-tooltip-shared-style](#project-tooltip-shared-style) — Tooltip style is shared via shared/tooltip/constants.toml (per-driver alphas border_alpha_hs vs _ahk are intentionally different); the macOS stacked canvas rounds its colored rows via an hs.canvas clip probe with CLIP_OFFSET index shift
...
**Rounding the stacked colored rectangle.** The single tooltip (`renderer.M.canvas`) always rounded its background, but the stacked canvas drew one sharp-cornered fill rectangle PER ROW under a rounded border — a colored rectangle poking outside the rounded border. hs.canvas cannot round only the outer corners of a multi-rect stack, so the fix is a leading `action="clip"` rounded rectangle (the macOS analog of the AHK `CreateRoundRectRgn` window region) plus a trailing `action="resetClip"` so the 1 px border still draws crisp. Support is probed ONCE at load (`STACKED_CLIP`) so an older Hammerspoon build degrades to square row corners instead of a hidden tooltip; the clip adds `CLIP_OFFSET` (0 or 1) to every stacked element index — keep `_ensure_stacked_canvas` and `render_stacked` index math in lockstep. The stub `hs.canvas` is a no-op mock, so the stacked element structure is NOT unit-testable — visual verification on a real Mac is required after touching it.
