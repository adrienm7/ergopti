...
| 🔄 | **P9.4** | Conversion behavior (bucket A) · 226 AHK + 128 macOS | *Audit lot-0 (2026-06-29) :* sur les 238 tests AHK `_DriverFuncBody`, **~4-6 sont vraiment bucket-A** (fonctions pures appelables dans le runner headless) ; le reste sont correctement structurels (bucket-B) — ils testent des invariants async/threading/OS (`A_IsSuspended`, `ih.Wait()`, WinEvent, `Critical`) qui ne peuvent pas être stimulés dans un runner synchrone sans mocker des built-ins AHK non mockables. La métrique `introspection_ratio ≤ 0,25` n'est **pas réaliste** sans infrastructure de mocking AHK ou refactoring de prod. **À faire** : convertir les tier-1 identifiés (`test_dispatcher_register_duplicate_label`, `test_llm_instant_word_end_trigger`) ; pour macOS, les tests `driver_root() ..` qui font du `src:find` sur du code comportemental (ex. sections 4 de `test_api_ollama.lua`). Chaque conversion = behavior rouge-avant/vert-après. `AHK`/`HS`. **Faible** (tests-only) |
