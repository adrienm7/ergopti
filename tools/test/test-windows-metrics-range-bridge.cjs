...
assert.match(host, /SetTimer\(KLWV_PushRangeData\.Bind\(which, query\), -1\)/,
	'Windows must defer heavy range reads until after the WebView callback returns');
assert.match(host, /KLR_ReadRangeSplitToday\(db, query\["start_date"\], query\["end_date"\], query\["apps"\]\)/,
	'Windows range projection must honour the exact date and app selection');
assert.match(host, /"type":"range_data"/,
	'Windows host must identify the response as range_data');
