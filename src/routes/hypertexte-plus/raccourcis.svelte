...
<ul class="margin-top-2">
	...
	<p>
		...
		à la fin d’un "mot". Ainsi, Capslock se désactive lors de la pression de <kbd>Espace</kbd>,
		<kbd>,</kbd> ou encore <kbd>Entrée</kbd>. Cette fonctionnalité peut être très utile pour écrire
		des abréviations telles que <kbd-sortie>QMK</kbd-sortie>, <kbd-sortie>ASCII</kbd-sortie>,
		<kbd-sortie>FAQ</kbd-sortie>, etc. De plus, elle permet également d’écrire plus aisément les
		noms des variables et constantes en programmation comme <kbd-sortie>KC_SPC</kbd-sortie>,
	...
	<p>
		...
		emplacement, car la touche <kbd-sortie>Suppr.</kbd-sortie> est quant à elle disponible en
		<kbd>Shift</kbd>
		+ <kbd>"Capslock"</kbd>.
	</p>
	<li>
	...
	<p>
		...
		<kbd-sortie>Shift</kbd-sortie>
		pour la prochaine touche pressée, avant de se désactiver. C’est un gain de temps et de confort, car
		en un seul raccourci, trois touches sont pressées, dont la touche <kbd>Shift</kbd> qui est peu
		confortable à presser sur claviers standards (car pressée par les auriculaires). Un autre
		avantage est que <kbd>Shift</kbd> ne s’active que pour le premier caractère de la nouvelle
...
<p>
	...
	lors de son maintien. C’est particulièrement adapté pour les touches modificatrices (<kbd
		>Shift</kbd
	>,
	<kbd>Control</kbd>, <kbd>Alt</kbd>, <kbd>AltGr</kbd>). Ainsi, il est possible d’utiliser la touche
	<kbd>AltGr</kbd> normalement, tout en y assignant une autre fonction lors d’un simple appui, par
	exemple
...
<ul class="margin-top-2">
	...
	<p>
		...
		certaines consonnes. Toutefois, elle est peu accessible sur un clavier standard, nécessitant que
		l’auriculaire droit traverse 3 colonnes de lettres avant de l’atteindre. Par conséquent, placer
		cette touche sur la touche <kbd>AltGr</kbd> la ramène à un endroit extrêmement accessible. Cela
		supprime aussi beaucoup de <SFB />s.
	</p>
	<p>
		À noter que les raccourcis comme <kbd>Ctrl</kbd> + <kbd>Entrée</kbd> par exemple ne sont pas
		pris en charge par cette nouvelle touche <kbd>Entrée</kbd>. En effet, techniquement la touche
		<kbd>AltGr</kbd>
		équivaut à presser
		...
		raccourcis <kbd>Ctrl</kbd> + <kbd>Enter</kbd>, il faudra donc se rabattre sur la vraie touche
	...
	<p>
		...
		des raccourcis <kbd>Ctrl</kbd> + <kbd>Flèche</kbd>, <kbd>Ctrl</kbd> + <kbd>Shift</kbd> +
		...
		<kbd>Page Up</kbd>, <kbd>Page Down</kbd>, <kbd>Home</kbd> et <kbd>End</kbd> sont ajoutés. Ce layer
		de navigation vous évite donc des positions inconfortables nécessitant de presser plusieurs modificateurs
