--- tests/unit/modules/karabiner/test_no_fd_leak_in_first_launch.lua

--- ==============================================================================
--- MODULE: Karabiner FD-Leak Regression Test
--- DESCRIPTION:
--- Regression guard ensuring that the first_launch file-existence check in
--- modules/karabiner/init.lua never leaks an open file descriptor.
---
--- FEATURES & RATIONALE:
...
--- Reads the source of modules/karabiner/init.lua relative to driver_root.
...
local function read_karabiner_init_source()
	...
	helpers.assert_true(body ~= nil, "modules/karabiner/init.lua source must be locatable")
	return body
end
...
helpers.describe("karabiner/init.lua: no fd-leak in first_launch detection", function()
	...
	helpers.it("source file is readable (precondition)", function()
		helpers.assert_true(src ~= nil, "could not open modules/karabiner/init.lua for reading")
	end)
