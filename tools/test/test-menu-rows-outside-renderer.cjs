...
const BASELINE = {
	...
	// 42 → 31: the tray root. Every component builder ended with the row that
	// hangs its submenu on the menu bar — written by hand once per submenu, a dozen
	// times over — and generate() returned that array untouched. It is row data now
	// and the shared renderer draws it, exactly as Linux has done since the day
	// before. It also FIXED two entries: menu_remap and menu_keyboard_layout had
	...
	// 15 → 3. The `toggle` type was DECLARED in the manifest and skipped by the
	// shared renderer with the comment "rendered by caller" — true of exactly one
	// driver, since the AutoHotkey renderer has built it from the declaration all
	// along. This driver rebuilt the same row by hand in four submenus, reading the
	// same two i18n keys the declaration names. The renderer builds it now, and
	// whether a tray NEEDS the row is answered by REGISTERING THE COMMAND rather
	// than by a second declaration: a tray whose parent can be clicked registers
	// none and gets nothing.
	//
	// With it went the reload row, the migration readout and the extension-pack
	// rows — all three declared with their reason, since they are this driver's
	// own. Three rows remain: the two of the degraded tray, which exists for the
	// case where the menu subsystem failed to load and must not depend on it, and
	// the one shared by the two `dynamic` WPM rows, where appending in the driver
