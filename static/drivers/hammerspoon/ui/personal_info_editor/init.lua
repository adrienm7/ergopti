...
-- ===========================================================================
-- Personal Information Editor UI.
--
...
local function read_asset(filename)
    ...
    if not ok or not fh then
        print("[personal_info_editor] Error loading asset from: " .. tostring(path))
        return ""
    end
...
function M.open(current_info, save_callback)
    ...
    if _srv then
        print("[personal_info_editor] Server already running on port " .. SERVER_PORT)
    ...
    if ok and h_srv then
        ...
        pcall(hs.execute, string.format("open \"http://127.0.0.1:%d/\"", SERVER_PORT))
        print("[personal_info_editor] Editor server active on port " .. SERVER_PORT)
    else
        print("[personal_info_editor] Failed to initialize local HTTP server.")
    end
end
