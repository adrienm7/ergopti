...
- [~] **Extraire les algos inline d'`init.lua`** (découverte hotstrings, cleanup MLX, scan personal-hotstrings, file-watcher) → modules/lib ; renuméroter les bannières ; renommer le `boot_llm_enabled` dupliqué. ⚠️ Sensible à l'ordre de boot (comme l'entrée Windows P4) — incréments vérifiés.
  ...
  - [ ] **Découverte hotstrings** (`safe_dir_entries`, `has_common_hotstring_groups`, `HOTSTRINGS_EXCLUDED_STEMS`), **scan personal-hotstrings**, **file-watchers** — **différé** : nécessite un reload Hammerspoon incrémental (la suite unitaire ne charge jamais `init.lua`). `safe_dir_entries` est en plus dupliqué dans `ui/hotstrings_config_window/init.lua` et gardé par un meta-test (`test_fs_dir_iterator_contract`) qui épingle la régression de crash-au-boot *fsdir-drops-state* sur la source d'`init.lua` ; les watchers ont des contraintes de GC-rooting à vérifier en live. À faire en session dédiée reload-vérifiable, pas à l'aveugle.
