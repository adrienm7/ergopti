...
**Driver** — L'une des trois implémentations complètes d'ErgoptiPlus : `windows/` (AutoHotkey v2), `macos/` (Hammerspoon/Lua) ou `linux/` (Lua/LuaJIT). Les trois partagent données et frontends via `_shared/`.
