--- tests/unit/modules/karabiner/test_defaults_shared_path_resolver.lua
...
helpers.describe("init.lua: the Karabiner require cannot abort boot", function()
	...
	helpers.it("requires modules.karabiner through pcall", function()
		...
		helpers.assert_true(src:find('local karabiner%s*=%s*require%("modules%.karabiner"%)') == nil,
			"a bare `local karabiner = require(\"modules.karabiner\")` is back — that is "
