...
in `macos/platform/remap/data/mod_combos.json` and configured via
`platform/remap/init.lua`. Distinct from **→ Tap-Hold**.
...
action dictionary. See `macos/platform/remap/data/mod_combos.json`.
...
(`hold_layer`). See `_shared/tap_hold/defaults.toml`.
...
via `platform/remap/generator.lua`.
...
in `_shared/tap_hold/defaults.toml` under `[tap_hold.layers]` and implemented
in the Karabiner JSON generated by `platform/remap/generator.lua`.
...
`time_activation_seconds` in `_shared/tap_hold/defaults.toml`. Test vectors
