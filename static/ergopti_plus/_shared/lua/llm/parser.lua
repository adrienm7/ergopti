--- _shared/lua/llm/parser.lua

--- ==============================================================================
--- MODULE: LLM Output Parser — Shared Lua Implementation
--- DESCRIPTION:
...
--- the _shared/tests/corpus/llm/process_prediction_vectors.json golden corpus.
---
...
--- threaded in through the `opts` table of process_prediction (the macOS shim
...
--- @param opts table|nil Caller-resolved limits: { min_words = number, max_words = number }.
---        Both default to 0 ("no limit") when absent. The macOS shim fills these
---        from its settings store; this keeps the parser free of platform coupling.
--- @return table|nil A table containing prediction data or nil if invalid.
function M.process_prediction(full_text, tail_text, block, opts)
	Logger.debug(LOG, "Parsing model output block…")

	-- The min/max word limits are the only OS-specific inputs the parser needs;
	-- the caller resolves them (the macOS shim from its settings store) and threads
	-- them in so this module stays platform-neutral. 0 means "no limit", matching
	-- the enforce_word_limits guard below.
	opts = opts or {}
