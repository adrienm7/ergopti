...
if [[ "$APP_FAMILY" == "teams" ]]; then
	# Teams must be launched via LaunchServices (`open -n -a`) — launching its
	# binary directly causes it to silently exit (integrity / singleton checks).
	# `open` unfortunately strips __CFBundleIdentifier from the env, so the
	# Dock tile won't show the tinted icon while Teams is running; that is an
	# accepted trade-off versus the app not opening at all.
	cat > "$LAUNCHER" <<LAUNCHEREOF
#!/bin/zsh
# Generated by AppCloner (teams family). Do not edit — regenerated on each clone.
set -e

OPEN_ARG="${OPEN_ARG}"
URL_SCHEME_RE='^[a-zA-Z][a-zA-Z0-9+.-]*://'

# Launch Teams through LaunchServices so it passes its own integrity checks.
# -n forces a new instance even if Teams is already running.
/usr/bin/open -n -a "${SOURCE_APP}"

if [[ -n "\$OPEN_ARG" ]]; then
	# Give Teams ~3 s to register its URL handlers before dispatching
	sleep 3
	/usr/bin/open -a "${SOURCE_APP}" "\$OPEN_ARG"
fi

# Keep the launcher alive so the Dock tile stays «running» until Teams quits.
# Poll every 2 s instead of a fixed sleep so we notice Teams quitting quickly.
while /usr/bin/pgrep -qf "${SRC_EXE_NAME}" 2>/dev/null; do
	sleep 2
done
LAUNCHEREOF
	chmod +x "$LAUNCHER"
	log "launcher written (teams/open -n -a path for ${SRC_EXE})"
else
	cat > "$LAUNCHER" <<LAUNCHEREOF
#!/bin/zsh
