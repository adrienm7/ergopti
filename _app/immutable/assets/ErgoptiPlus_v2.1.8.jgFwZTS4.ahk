...
#HotIf pilote_azerty and !layer_actif
...
SC01D & ~SC138::
{
    tap := KeyWait("SC138", "T.25")
    if (tap and A_PriorKey == "RAlt") { ; Si la touche est relâchée avant le temps d’attente
        desactiver_capsword()
...
#SC01E:: selection_ligne() ; Win + A (All) pour sélectionner toute la ligne

#SC018:: Send("+#s") ; Win + C (Capture) pour faire une capture d’écran
...
; Si le texte est déjà en majuscules, pasasge en minuscule
{
    ...
    if (RegExMatch(A_Clipboard, "[a-z]")) {
        Send("{Raw}" Format("{:U}", A_Clipboard))
    } else {
        Send("{Raw}" Format("{:L}", A_Clipboard))
    }
    Sleep(50)
    A_Clipboard := ancien_clipboard ; Restore the original clipboard. Note the use of Clipboard (not ClipboardAll).
    ancien_clipboard := "" ; Free the memory in case the clipboard was very large.
}
...
#SC027::#r ; Win + R (Run) pour ouvrir la boîte de dialogue Exécuter. Si ce n’est pas défini explicitement, on aura étrangement le comportement de #SC013
...
selection_mot() {
    Send("{Left}") ; Pour être sûr d’être sur le mot
    Send("^{Right}{Shift Down}{Left}{Shift Up}") ; Sélection du caractère juste après
    ancien_clipboard := ClipboardAll()
    Send("^c")
    Sleep(200)
    Send("{Left}")
    caractere := A_Clipboard
    if (caractere = " ") {
        Send("^{Left}{Shift Down}^{Right}{Left}{Shift Up}")
    } else {
...
:*?:elleance::ellance
:*?:elleement::ellement
:*?:automatiqueement::automatiquement
