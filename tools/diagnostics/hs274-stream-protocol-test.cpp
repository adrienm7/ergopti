...
int main(int argc, char** argv) {
  ...
  auto interrupted_open = owner.request(7, open);
  pull["lease"] = interrupted_open.at("lease");
  owner.append({1, 1, 1, true, true, 7, 44});
  ...
  require(!interrupted.contains("records"));
  require(interrupted.at("lease") == interrupted_open.at("lease"));
  require(interrupted.at("incarnation") == "first-process");
  owner.peer_closed(7);
  auto after_interrupt = owner.request(7, open);
  require(after_interrupt.at("lease") != interrupted_open.at("lease"));
  rejects([&] { owner.request(7, pull); });
  pull.erase("ack");
  pull["lease"] = after_interrupt.at("lease");
  require(owner.request(7, pull).at("records").empty());
