...
; ── LLM tooltip UI chrome (shared/tooltip/constants.toml [llm_ui]) ───────────
...
_UiStyleFatal(section, key, detail := "") {
	msg := Format("Erreur fatale : shared/tooltip/constants.toml — clé manquante ou invalide : [{1}] {2}", section, key)
	if (detail != "")
		msg .= "`n" . detail
...
/**
 * Reads shared/tooltip/constants.toml at startup and assigns every tooltip
