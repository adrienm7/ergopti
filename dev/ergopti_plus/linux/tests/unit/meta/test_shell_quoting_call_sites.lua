...
helpers.describe("shell quoting at the call sites", function()
	...
	helpers.describe("network_info.getSsidHash()", function()
		helpers.it("quotes the SSID before hashing it", function()
			-- The SSID is named by whoever owns the access point, so it is the
			-- one string on this path an attacker fully controls.
			local net = helpers.load_module("adapters.network_info")
			local cmds = capture(function() net.getSsidHash() end, function(cmd)
				if cmd:find("iwgetid", 1, true) then return PAYLOAD .. "\n" end
				return ""
			end)
			assert_posix_quoted(find_command(cmds, "openssl"), "network_info.getSsidHash")
		end)
	end)


	helpers.describe("clipboard.write()", function()
		helpers.it("quotes the clipboard content it pipes to the backend", function()
			-- The backend is detected at require time, so the module is loaded
			-- with the seam already installed: otherwise a host without xclip
			-- composes no command at all and the case would prove nothing.
			local cmds = capture(function()
				local clipboard = helpers.load_module("adapters.clipboard")
				clipboard.write(PAYLOAD)
			end, function() return "" end)
			assert_posix_quoted(find_command(cmds, "printf"), "clipboard.write")
		end)
	end)


	helpers.describe("window_manager", function()
		helpers.it("quotes the window spec it searches for", function()
			local wm = helpers.load_module("adapters.window_manager")
			local cmds = capture(function() wm.exists(PAYLOAD) end)
			assert_posix_quoted(find_command(cmds, "xdotool search"), "window_manager.exists")
		end)

		helpers.it("quotes the window spec it closes", function()
			local wm = helpers.load_module("adapters.window_manager")
			local cmds = capture(function() wm.kill(PAYLOAD) end)
			assert_posix_quoted(find_command(cmds, "wmctrl"), "window_manager.kill")
		end)
	end)


	helpers.describe("app_launcher", function()
		helpers.it("quotes the executable path it launches", function()
			local al = helpers.load_module("adapters.app_launcher")
			local cmds = capture(function() al.AL_Launch(PAYLOAD) end)
			local cmd = find_command(cmds, "nohup")
			assert_posix_quoted(cmd, "app_launcher.AL_Launch")
			-- The raw form was "nohup %s": a path with a space started the
			-- wrong program long before any metacharacter mattered.
			helpers.assert_contains(cmd, "nohup " .. Shell.quote(PAYLOAD),
				"the path must be one word, not several")
		end)

		helpers.it("quotes the executable path when args are appended", function()
			local al = helpers.load_module("adapters.app_launcher")
			local cmds = capture(function() al.AL_LaunchWithArgs(PAYLOAD, "--flag") end)
			local cmd = find_command(cmds, "nohup")
			assert_posix_quoted(cmd, "app_launcher.AL_LaunchWithArgs")
			helpers.assert_contains(cmd, "--flag",
				"args stay verbatim by contract — quoting them would collapse a whole argument list into one word")
		end)

		helpers.it("quotes the process name it probes", function()
			local al = helpers.load_module("adapters.app_launcher")
			local cmds = capture(function() al.AL_IsRunning(PAYLOAD) end)
			assert_posix_quoted(find_command(cmds, "pgrep"), "app_launcher.AL_IsRunning")
		end)
	end)


	helpers.describe("notifier.send()", function()
		helpers.it("quotes the title and the body through the shared quoter", function()
			-- The notifier owns its own seam (its io.popen is fire-and-forget),
			-- so it is driven through that one rather than the shell_runner's.
			local notifier = helpers.load_module("adapters.notifier")
			local seen
			notifier._set_runner(function(cmd) seen = cmd end)
			local ok, err = pcall(function() notifier.send(PAYLOAD, { body = PAYLOAD }) end)
			notifier._reset_runner()
			if not ok then error(err, 0) end
			assert_posix_quoted(seen, "notifier.send")
		end)
	end)


	helpers.describe("backend detection is interpreter-independent", function()
		helpers.it("the clipboard picks a backend whenever the probe succeeds", function()
			-- A successful probe used to be recognised only as the exit code 0.
			-- Lua 5.2+ reports it as `true`, so on any modern interpreter every
			-- probe read as "absent" and the adapter silently no-opped — a class
			-- of failure CI cannot see, because CI runs LuaJIT.
			local backend_probes = {}
			Shell._set_runner(function(cmd)
				if cmd:find("command -v", 1, true) then backend_probes[#backend_probes + 1] = cmd end
				return ""
			end)
			local clipboard = helpers.load_module("adapters.clipboard")
			local wrote = clipboard.write("x")
			Shell._reset_runner()
			helpers.assert_true(#backend_probes > 0, "the adapter must probe for a clipboard tool")
			helpers.assert_eq(wrote, true,
				"a probe reported as success must yield a usable backend on this interpreter")
		end)

		helpers.it("the clipboard reports no backend when the probe fails", function()
			-- The other half of the invariant: a probe that genuinely fails must
			-- still disable the adapter, so the case above cannot be passing
			-- merely because the seam forces everything to succeed.
			Shell._set_runner(function() return false end)
			local clipboard = helpers.load_module("adapters.clipboard")
			local wrote = clipboard.write("x")
			Shell._reset_runner()
			helpers.assert_eq(wrote, false, "no tool means no clipboard, not a silent success")
		end)
	end)
