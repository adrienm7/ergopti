--- tests/unit/meta/test_corpus_updater_release_parser.lua

--- ==============================================================================
--- MODULE: Updater Release Parser Corpus Consumer (macOS)
--- DESCRIPTION:
...
---
--- This pins the shared parser (parse_tag, parse_notes, parse_asset_url,
--- parse_prerelease_flag, split_releases_array, parse_html_url,
--- parse_published_at) so any divergence between the Lua and AHK
...
local corpus_path = helpers.shared("tests/corpus/updater/release_parser_vectors.json")
...
local function dispatch(P, vec)
	...
	if cat == "parse_tag" then
		return P.parse_tag(input.body)

	elseif cat == "parse_notes" then
		return P.parse_notes(input.body)

	elseif cat == "parse_asset_url" then
		return P.parse_asset_url(input.body, input.asset_name)

	elseif cat == "parse_prerelease_flag" then
		return P.parse_prerelease_flag(input.body)

	elseif cat == "parse_html_url" then
		return P.parse_html_url(input.body)

	elseif cat == "parse_published_at" then
		return P.parse_published_at(input.body)

	elseif cat == "split_releases_array" then
		local chunks = P.split_releases_array(input.json)
		return #chunks
	end
...
helpers.describe("updater release parser corpus — vector replay", function()
	...
	for _, vec in ipairs(by_cat["split_releases_array"] or {}) do
		helpers.it("split_releases_array: " .. vec.id, function()
			local result = dispatch(P, vec)
			helpers.assert_eq(result, vec.expected_count,
