...
local function post_and_parse(model_name, system_prompt, full_text, tail_text,
	...
	Logger.debug(LOG, "Preparing request to MLX server…")
	...
	if line_mode then
		...
		payload = {
			-- MLX server identifies loaded model via "default_model" when field is absent;
			-- sending Ollama name (e.g. "llama3.2:3b") triggers HF validation → HTTP 404
			messages    = messages,
			stream      = false,
			temperature = opts.temperature,
