...
- [ ] **Splitter les 2 god-files MLX** : `modules/llm/api_mlx.lua` (1802 l.) + ~~`ui/menu/menu_llm/models_manager_mlx.lua` (1790 l.)~~. *(Vérifiable par la suite unitaire Lua ; reload Hammerspoon par le mainteneur pour confirmer, comme les splits AHK P4/P5.)*
  ...
  - [~] `modules/llm/api_mlx.lua` : **1038 l.** (le « 1802 » du plan était périmé). Déjà décomposé en frères `api_mlx_inference.lua` (721 l. — mécanique requête/streaming) + `api_mlx_fetch.lua` (259 l. — stratégies batch/parallel/sequential). Le reste (`warmup` ~200 l., `discover_endpoints` ~200 l.) est fortement couplé à l'état du module (clients HTTP dédiés, cache d'endpoints, `MLX_BASE_URL`, flags `_warmup_in_flight`) → extraction possible mais à faible gain et sur du code backend critique. Laissé tel quel pour l'instant.
