...
SWIFTEOF
	...
	# The PWA launcher. It is the stub bundle's main executable — stays alive
	# so the Dock tile runs under our bundle-id, not Edge's.
	#
	# Singleton guarantee: we use a lock file containing our PID. On every
	# launch we check whether that PID is alive before writing a new one.
	# macOS may relaunch this binary when the user clicks the Dock tile even
	# while we are running (Dock does not always coalesce clicks for shell-script
	# bundles). The early-exit path handles that second instance immediately.
	cat > "$LAUNCHER" << PWAEOF
#!/bin/zsh
