...
_PromptEdWeb_Save(Name, Batch, Prompt) {
	global _LLM_Menu, _PromptEdWeb_EditId

	if (_PromptEdWeb_EditId != "") {
		for i, p in _LLM_Menu["user_profiles"] {
			if (p.Has("id") && p["id"] == _PromptEdWeb_EditId) {
				_LLM_Menu["user_profiles"][i]["label"]         := Name
				_LLM_Menu["user_profiles"][i]["system_single"] := Prompt
				_LLM_Menu["user_profiles"][i]["batch"]         := Batch
				break
			}
		}
	} else {
