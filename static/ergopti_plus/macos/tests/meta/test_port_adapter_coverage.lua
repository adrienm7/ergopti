...
local LUA_HS_BASELINE       = 943  -- hs.* calls in macos/modules/ and macos/lib/ (943: +6 vs 937 — audit bug-fix batch: perf.lua lib-logger-perf-001 added 2 defensive comment lines + 1 guard + 1 pcall referencing hs.timer (+4 net); system.lua shortcuts-core-2 added 1 hs.* call; gestures/actions.lua added 1 hs.* call. 937: +5 vs 932 — A6-v2 audit fixes: readAllData/writeAllData in keymap/utils.lua for non-text clipboard preservation; processInfo.processID + eventSourceUnixProcessID + keycodes.map["v"] in keymap/init.lua for synthetic-backspace PID guard and paste-counter Cmd+V check. 932: +10 vs 922 — audit fixes added boundary-layer hs.* calls in api_ollama/_infer+_check client split, api_remote split, gestures/engine emergency_reset, karabiner/watchers nil-guards; pending adapter extraction. 922: +2 vs 920 — karabiner/init.lua gained 2 direct hs.* calls for the _wizard_ran_this_session guard and hs.timer.doAfter to prevent re-showing the install dialog on hs.reload(). 920: +7 vs 913 — gesture engine.lua added 7 direct hs.* calls for touch-device integration. 913: +1 vs 912 — rules_engine.lua emit callback inside inject_dynamic has an hs.eventtap.keyStrokes fallback for when km_utils is unavailable; rules_engine is the injection boundary and this matches the pre-existing fallback pattern. 912: +1 vs 911 — i18n.persist_locale writes the chosen locale to the settings store so it survives the onboarding wizard's reload; lib/i18n already persists the locale via the same settings API directly, so this one extra call is consistent with that boundary. 911: +2 vs 909 — the keep-awake jiggler in shortcuts/actions/system.lua posts an F18 no-op key (newKeyEvent down+up) every tick so the HID idle counter resets and Teams stays "available"; warping the cursor alone does not. system.lua is the OS-actions boundary and already posts events directly, so these two synthetic-key posts are legitimate. Earlier: +3 vs 906 from A5/A6/E2 audit timing fixes.)
