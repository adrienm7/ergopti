...
const CHECKS = [
	...
	{ name: 'no new menu row built outside the renderer (I3 ratchet — windows 222, macos 301, linux 3)', cmd: 'node', args: ['tools/test/test-menu-rows-outside-renderer.cjs'], repro: 'node tools/test/test-menu-rows-outside-renderer.cjs' },
	{ name: 'every declared menu row has a handler in its driver (I3 ratchet — ahk 0, hs 20, linux 27)', cmd: 'node', args: ['tools/test/test-menu-action-handler-bijection.cjs'], repro: 'node tools/test/test-menu-action-handler-bijection.cjs' },
	...
	{ name: 'no new driver-namespaced manifest table (I2 ratchet — 223 today, renaming them is a config-schema break)', cmd: 'node', args: ['tools/test/test-feature-namespace-ratchet.cjs'], repro: 'node tools/test/test-feature-namespace-ratchet.cjs' },
	{ name: 'no new unexplained platform restriction (I2 ratchet — 76 today; run with --report for the inventory)', cmd: 'node', args: ['tools/test/test-platform-restrictions-explained.cjs'], repro: 'node tools/test/test-platform-restrictions-explained.cjs --report' },
	...
	{ name: 'file watchers constants single source (SCAN_MAX_DEPTH=16 + debounce=0.5s match across Linux + macOS lib/file_watchers.lua)', cmd: 'node', args: ['tools/test/test-file-watchers-constants-single-source.cjs'], repro: 'node tools/test/test-file-watchers-constants-single-source.cjs' },
