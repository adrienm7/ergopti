// static/ergopti_plus/_shared/core/ports/KeyState.spec.js
 ...
 * 2. Dual helpers: isDown and isUp are both required so call sites read
...
const portContract = {
	...
	/**
	 * isDown(keyName) — Returns true when the key is physically held down.
	 ...
	 *
	 * isUp(keyName) — Returns true when the key is not physically held down.
	 *   Equivalent to !isDown(keyName).
	 ...
	 */
	methods: {
		isDown: { arity: 1, required: true },
		isUp: { arity: 1, required: true }
	}
};
...
function contractTestVectors() {
	return [
		...
		},
		{
			...
			description: 'isUp() with an unknown key name returns true (not pressed = up).',
			steps: [{ call: 'isUp', args: ['ERGOPTI_NONEXISTENT_KEY_XYZ'] }, { assert: 'return_true' }]
		},
		{
			id: 'is_down_is_up_are_inverse',
			description: 'isDown and isUp return opposite values for the same key.',
			steps: [
				{ call: 'isDown', args: ['LShift'] },
				{ call: 'isUp', args: ['LShift'] },
				{ assert: 'return_inverse_pair' }
			]
		},
		{
			id: 'is_down_returns_boolean',
			description: 'isDown() returns a boolean, never throws.',
			steps: [
				{ call: 'isDown', args: ['SC038'] },
				{ assert: 'return_type_one_of', expected: ['boolean'] }
			]
		},
		{
			id: 'is_up_returns_boolean',
			description: 'isUp() returns a boolean, never throws.',
			steps: [
				{ call: 'isUp', args: ['SC038'] },
				{ assert: 'return_type_one_of', expected: ['boolean'] }
			]
		}
	];
