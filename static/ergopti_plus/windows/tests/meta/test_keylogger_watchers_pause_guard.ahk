...
_KLLG_IdleTickHasPauseGuard() {
	...
	Seg := _KLLG_FuncBody(Src, "KL_Watchers_IdleTick() {")
...
_KLLG_OnSessionChangeHasPauseGuard() {
	Src := _KLLG_ReadSource("modules/keylogger/keylogger_watchers.ahk")
	Seg := _KLLG_FuncBody(Src, "KL_Watchers_OnSessionChange(wParam, lParam, msg, hwnd) {")
...
_KLLG_OnPowerBroadcastHasPauseGuard() {
	Src := _KLLG_ReadSource("modules/keylogger/keylogger_watchers.ahk")
	Seg := _KLLG_FuncBody(Src, "KL_Watchers_OnPowerBroadcast(wParam, lParam, msg, hwnd) {")
