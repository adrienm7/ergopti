...
-- Selected by a declaration unique to infra/ui_restore.lua rather than by
-- path, so moving or splitting the module cannot turn this invariant
-- into a path error.
local src = helpers.read_driver_source("local function any_ui_open")
helpers.assert_true(src ~= nil, "infra/ui_restore.lua source must be locatable")

-- Test 1: _reload_in_flight flag declared at module level.
local flag_pos = src:find("local _reload_in_flight", 1, true)
helpers.assert_true(
	flag_pos ~= nil,
	"infra/ui_restore.lua must declare _reload_in_flight at module level (lib-misc-defer)"
)
...
helpers.assert_true(
	fast_guard ~= nil,
	"infra/ui_restore.lua fast path must check _reload_in_flight (lib-misc-defer)"
)
...
helpers.assert_true(
	set_true ~= nil,
	"infra/ui_restore.lua must set _reload_in_flight = true before calling the reload fn (lib-misc-defer)"
)
...
helpers.assert_true(
	set_false ~= nil,
	"infra/ui_restore.lua must reset _reload_in_flight = false after calling the reload fn (lib-misc-defer)"
)
