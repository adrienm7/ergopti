...
function M.build_management(ctx)
    ...
    table.insert(delay_menu, {
        ...
        fn       = not paused and function()
            ...
            state.delays = {}
            if ctx.keymap then
                if type(ctx.keymap.set_base_delay) == "function" then pcall(ctx.keymap.set_base_delay, def_base) end
                if type(ctx.keymap.set_delay) == "function" then
                    for k, v in pairs(def_delays) do pcall(ctx.keymap.set_delay, k, v) end
                end
            end
            ctx.save_prefs()
            ctx.updateMenu()
