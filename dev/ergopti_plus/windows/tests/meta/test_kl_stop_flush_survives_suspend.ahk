...
;
; The fix introduces Keylogger._shutting_down, set to true in KL_Stop() before
; the flush calls. The two suspend guards are amended to bypass on that flag so
; the shutdown flush is always allowed through regardless of pause state.
;
...
_KSFS_KLStopSetsFlag() {
	...
	Assert(FlagIdx > 0,
		"KL_Stop must set Keylogger._shutting_down := true before flushing (kl-stop-flush-defeated-by-suspend)")
	Assert(FlushIdx > 0,
		"KL_Stop must call KL_FlushBuffer() (kl-stop-flush-defeated-by-suspend)")
	Assert(FlagIdx < FlushIdx,
		"KL_Stop must set _shutting_down BEFORE calling KL_FlushBuffer — flag must be up when the flush guard runs (kl-stop-flush-defeated-by-suspend)")
}
Test("keylogger: KL_Stop() sets _shutting_down before KL_FlushBuffer (kl-stop-flush-defeated-by-suspend)", _KSFS_KLStopSetsFlag)
