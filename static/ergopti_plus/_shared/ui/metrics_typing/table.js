// _shared/ui/metrics_typing/table.js
...
function render_current_tab() {
	...
	const source_dict =
		app_state.current_tab === 'mods' ? derive_modifier_usage(source_dict_raw) : source_dict_raw;
	const pause_thresh =
		parseInt(document.getElementById('pause_threshold')?.value ?? '2000', 10) || 2000;
	...
	Object.keys(effective_source).forEach((k) => {
		...
		const acc =
			k.includes('[BS]') || k.toLowerCase() === 'backspace'
				? 100
				: item.count > 0
					? ((item.count - item.errors) / item.count) * 100
					: 0;
		...
		const display_len =
			app_state.current_tab === 'kc' ? 1 : Math.max(1, Array.from(norm_key).length);
		...
		data_arr.push({
			...
			wpm:
				!['sc', 'sc_bg', 'sc_tg', 'sc_qg', 'sc_pg', 'mods'].includes(app_state.current_tab) &&
				!isNaN(avg) &&
				avg >= 10 &&
				avg <= pause_thresh
	...
	if (app_state.current_tab !== 'mods' && app_state.current_tab !== 'sc_bg') {
		...
		data_arr.forEach((item) => {
			const km_per_stroke = compute_key_km_per_stroke(
				item.key,
				char_km_map,
				is_kc_tab_flag,
				is_sc_tab_flag
	...
	if (occ_elem) {
		...
		if (app_state.current_tab === 'c') {
			...
			const mode_label =
				show_hs && show_llm
					? _t('ui_typing.table_mode_output')
					: show_hs || show_llm
						? _t('ui_typing.table_mode_mixed')
	...
	let g_time = 0,
		...
		g_acc_n = 0;
	data_arr.forEach((item) => {
		if (item.manual_count > 0 && item.avg > 0) {
			g_time += item.avg * item.manual_count;
			g_man += item.manual_count;
		}
		if (item.wpm > 0) {
			g_wpm += item.wpm;
			g_wpm_n++;
...
function render_kc_heatmap(kc_data_arr) {
	...
	const WIDE_KEYS = {
		36: 1.5, // return — top-row width on the QWERTY row (L-shape)
		48: 1.5, // tab — left edge at x=0, flush against Q
		51: 1.5, // backspace — 1.5u; right edge aligns with Return stem and r-shift (698 px)
		...
		49: 5.0, // space bar — fills cmd-L → cmd-R gap (exactly 5u wide)
		63: 1.0, // fn — 1u; right-anchored so left edge aligns with Esc/Tab/Shift
		59: 1.0, // ctrl-L
	...
	let min_x_px = Infinity,
		max_x_px = -Infinity;
	let min_y_px = Infinity,
		max_y_px = -Infinity;
	...
	const ROW_PX = Math.round(U * 0.9); // vertical distance between rows (px)
	...
	const heat_color = (count) => {
		...
		const t = Math.pow(count / max_count, 0.4);
		if (t < 0.5) {
			const tt = t * 2;
	...
	if (typeof _foreach_filtered_app === 'function') {
		_foreach_filtered_app((app) => {
			...
			Object.entries(kh).forEach(([kc_str, h]) => {
				const t =
					kc_hold_total[kc_str] || (kc_hold_total[kc_str] = { tap: 0, hold: 0, s: 0, n: 0, m: 0 });
				t.tap += h.tap || 0;
				t.hold += h.hold || 0;
				t.s += h.s || 0;
				t.n += h.n || 0;
	...
	key_entries.forEach(([kc_str, pos]) => {
		...
		const sy =
			half_slot === 'bottom' ? base_sy + Math.round(KH / 2) + Math.round(GAP / 2) : base_sy;
		const cx = sx + key_w / 2;
		const cy = sy + key_h / 2;
		...
		const is_special =
			SHORT_LABELS[kc_str] !== undefined && label_disp !== '' && label_disp !== 'Fn';
		...
		let tip_lines = [`<b>${escape_html(label_raw || kc_str)}</b> (kc ${kc_str})`];
		...
		if (mods) {
			const mod_sorted = Object.entries(mods)
				.sort((a, b) => b[1] - a[1])
				.slice(0, 4);
		...
		if (label_raw && label_raw.length === 1) {
			...
			if (follows) {
				const top3 = Object.entries(follows)
					.sort((a, b) => b[1] - a[1])
					.slice(0, 3);
				if (top3.length > 0) {
					tip_lines.push(`<hr style="border-color:#444;margin:3px 0">`);
					...
					top3.forEach(([ch, n]) => {
						const disp =
							ch === ' ' ? 'Espace' : ch === ' ' ? 'NBSP' : escape_html(ch.toUpperCase());
		...
		if (kc_str === '36') {
			...
			const stem_lx =
				compute_sx('42', pos_backslash) + off_x + compute_kw('42', pos_backslash) + GAP;
			const stem_w = 34; // ≈ 0.76u — Apple ISO Return stem is narrower than 1u
			const rx_ret = stem_lx + stem_w;

			const sy_qwerty =
				Math.round(((Y_TOP - 0.84) / Y_SPAN) * (5.5 * U)) - Math.round(KH / 2) + off_y;
		...
		tooltips +=
			`<div id="${tip_id}" class="hm-tooltip" style="display:none;position:fixed;z-index:9999;` +
...
function render_sfb_heatmap(sfb_by_kc, sfb_pairs_by_kc, kc_raw) {
	...
	let max_count = 1;
	Object.values(sfb_by_kc).forEach((c) => {
		if (c > max_count) max_count = c;
	});
	...
	key_entries.forEach(([kc_str, pos]) => {
		...
		const sfb_key_rate =
			key_occurrences > 0 ? ((sfb_key_count / key_occurrences) * 100).toFixed(1) : '0.0';
		...
		if (sfb_key_count > 0) {
			tip_lines.push(
				`<br>SFBs : ${format_number(sfb_key_count)} — taux : <b style="color:#f87171;">${sfb_key_rate}%</b>`
			);
		}
		...
		const all_rows = all_partner_kcs
			.map((pk) => ({
				partner_kc: pk,
		...
		if (all_rows.length > 0) {
			...
			all_rows.forEach(({ partner_kc: pk, count: p_count }) => {
				...
				const pct_of_key_sfb =
					sfb_key_count > 0 ? ((p_count / sfb_key_count) * 100).toFixed(0) : '0';
				const pct_key_use =
					key_occurrences > 0 ? ((p_count / key_occurrences) * 100).toFixed(1) : '0.0';
				const pct_sfb_total =
					sfb_total_all > 0 ? ((p_count / sfb_total_all) * 100).toFixed(2) : '0.00';
...
function build_searchable_key(raw_key, case_sensitive) {
	...
	if (app_state.current_tab === 'kc') {
		const kc_resolved =
			(window.keycode_layout && window.keycode_layout[raw_key]) || KEYCODE_NAMES[raw_key];
...
function render_table() {
	...
	app_state.render_timer = requestAnimationFrame(() => {
		...
		const case_sensitive_search = !!document
			.getElementById('btn_case_sensitive')
		...
		if (rows_elem) {
			rows_elem.innerHTML =
				arr.length > 0 ? `${format_number(arr.length)} ligne${arr.length > 1 ? 's' : ''}` : '';
		}
		...
		const is_shortcut_tab = ['sc', 'sc_bg', 'sc_tg', 'sc_qg', 'sc_pg', 'mods'].includes(
		...
		const html = arr
			.map((item, row_idx) => {
				...
				if (is_kc_tab) {
					...
					const has_custom_layout =
						window.keycode_layout && Object.keys(window.keycode_layout).length > 0;
					const kc_name =
						(has_custom_layout ? window.keycode_layout[item.key] : undefined) ??
					...
					const label =
						CONTROL_KEY_SYMBOLS[raw_lower] ??
						(is_nonprint
							? (CONTROL_CHAR_NAMES[single_code] ??
								`\\x${single_code.toString(16).toUpperCase().padStart(2, '0')}`)
							: null) ??
				...
				const avg_str = item.avg > 0 ? `${format_number(item.avg.toFixed(1))} ms` : '-';

				const wpm_color = item.wpm > 0 ? gradient_color(item.wpm, WPM_GRADIENT) : 'inherit';
				const wpm_str =
					item.wpm > 0 ? `${format_number(item.wpm.toFixed(1))} ${_t('ui_typing.unit_mpm')}` : '-';
				...
				if (!rep_available || item.rep === null) {
					...
					const rep_color =
						item.rep_rate >= 20 ? '#ff3b30' : item.rep_rate >= 10 ? '#ffcc00' : 'inherit';
		...
		tbody.innerHTML =
			html || `<tr><td colspan="8" style="text-align:center;">${_t('ui_typing.no_data')}</td></tr>`;
	});
}
