--- tests/support/activation_layout_fixture.lua

--- ==============================================================================
--- MODULE: Exact-Lease Activation Layout Fixture
--- DESCRIPTION:
--- Drives the real remap coordinator over token-aware activation doubles.
--- ==============================================================================
...
--- Runs remap construction and the complete activation scenario.
--- @param options table|nil Harness state overrides.
--- @param callback function Receives the real remap coordinator and observations.
function M.with_remap(options, callback)
	-- These real consumers capture the fixture's native host and collaborators.
	-- Restoring only the directly injected doubles leaves stale owners behind.
	return helpers.with_fresh_modules(REAL_MODULES, function()
		return with_remap(options, callback)
	end)
end
