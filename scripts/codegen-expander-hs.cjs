// scripts/codegen-expander-hs.cjs
// Generates static/drivers/hammerspoon/_generated/expander.lua from
...
const OUT_DIR  = path.resolve(__dirname, "../static/drivers/hammerspoon/_generated");
...
function generate() {
	...
	L("--- Source: static/drivers/_shared/domain/expander.spec.js");
	...
	L("\t\tprint(string.format(\"[expander] ERROR '%s' called before M.init() — registry not initialized.\", func_name))");
	L("\t\treturn false");
	L("\tend");
	L("\treturn true");
	L("end");
	...
	L("\t\tprint(\"[expander] ERROR M.init(): registry must expose mappings_for_tail — module non-functional.\")");
	L("\t\treturn");
	L("\tend");
	L("\tif _registry then");
