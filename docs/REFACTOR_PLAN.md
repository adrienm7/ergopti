...
- [~] Sondes `DllCall` (AltGr-Kana, scan magic-key) → `adapters/key_state.ahk` ✅ ; séquence de boot → `lib/boot.ahk` **toujours repoussé**.
  ...
  - [ ] *(toujours repoussé)* séquence de boot → `lib/boot.ahk` : extraire l'orchestration top-level toucherait l'ordre `#Include`/`#InputLevel`/exécution top-level (risque élevé, gain de lisibilité faible) ; le manifeste `#Include` + l'orchestration de boot sont le rôle légitime de l'entrée.
