...
_TLDT_DedupTickTime() {
	...
	Src := _TLDT_StripLineComments(_DriverDirConcat("infra"))

	; The static must be declared
	Assert(InStr(Src, "_LastErrTime") > 0,
		"infra/logger.ahk must declare _LastErrTime static to track when the last error was logged (logger-dedup-tick)")
