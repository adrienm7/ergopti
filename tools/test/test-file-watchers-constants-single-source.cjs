 ...
 *
 * ROOT CAUSE ENCODED:
 * The Linux `lib/file_watchers.lua` was written to mirror the macOS
 * `lib/file_watchers.lua` contract. Both share the same:
...
const LINUX = path.join(ROOT, 'static/ergopti_plus/linux/lib/file_watchers.lua');
const MACOS = path.join(ROOT, 'static/ergopti_plus/macos/lib/file_watchers.lua');
...
const linuxDepth = extractMaxDepth(linuxSrc, 'linux/lib/file_watchers.lua');
const macosDepth = extractMaxDepth(macosSrc, 'macos/lib/file_watchers.lua');
...
if (!linuxDebounceM) {
	errors.push('linux/lib/file_watchers.lua: _debounce_sec declaration not found');
} else {
	const linuxDebounce = parseFloat(linuxDebounceM[1]);
	if (linuxDebounce !== 0.5) {
		errors.push(`linux/lib/file_watchers.lua: _debounce_sec = ${linuxDebounce} — expected 0.5`);
	}
}
...
if (!macosDebounceM) {
	errors.push('macos/lib/file_watchers.lua: hs.timer.doAfter(0.5, ...) not found');
} else {
	const macosDebounce = parseFloat(macosDebounceM[1]);
	if (macosDebounce !== 0.5) {
		errors.push(`macos/lib/file_watchers.lua: doAfter delay = ${macosDebounce} — expected 0.5`);
	}
}

// ── 5. Ratchet: macOS comment must mention 0.5 s for documentation parity ──
if (!macosSrc.includes('0.5 s timer')) {
	errors.push('macos/lib/file_watchers.lua: missing debounce documentation ("0.5 s timer")');
}

// ── 6. Ratchet: Linux comment must mention "matches macOS" for traceability ──
if (!linuxSrc.includes('matches macOS')) {
	errors.push('linux/lib/file_watchers.lua: missing cross-reference comment ("matches macOS")');
}
