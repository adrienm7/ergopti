...
function M.build(ctx)
	...
	--- @param isAxis boolean True if the gesture uses an axis.
	...
	local function slotItem(slot, isAxis)
		...
		if type(names) == "table" then
			for _, aname in ipairs(names) do
				-- "--" sentinel in SG_NAMES (mirrored from AutoHotkey's
				-- GESTURE_ACTION_NAMES) materialises as a visual separator.
				if aname == "--" then
	...
	--- @param isAxis boolean True if the gestures use an axis.
