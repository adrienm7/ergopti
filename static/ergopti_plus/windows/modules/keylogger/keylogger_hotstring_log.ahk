 ...
 * @param {Func} publish_guard - Optional lifecycle predicate supplied by the
 *     deferred queue. It is re-checked at every mutation boundary.
 * @return {Boolean} True once the fire is consumed; false when lifecycle
 *     ownership expired before publication.
 */
KL_LogHotstring(trigger, replacement, h_type := "unknown", app_name := "", category := "", section := "", is_private := false, publish_guard := unset) {
	; This sink mutates three independent stores. The deferred queue owns a fire
	; only while the driver is live; native Suspend does not stop its timer, so the
	; guard must precede KL_FlushBuffer (the first destructive mutation), not live
	; solely in KL_AppendLog below. Shutdown is the explicit exception: its lease
	; persists pre-pause rows before the process loses the in-memory batch.
	if A_IsSuspended && !Keylogger._shutting_down
		return false
	if IsSet(publish_guard) && !publish_guard.Call()
		return false
	...
	if !Keylogger.initialized
		return true
	if IsSet(publish_guard)
		KL_FlushBuffer(publish_guard, &FlushDeferred)
	else
		KL_FlushBuffer(, &FlushDeferred)
	; Another detached flush owns the older typing snapshot. Retain this fire and
	; retry after that owner publishes, otherwise its canonical row can overtake
	; the keystrokes that preceded it.
	if FlushDeferred
		return false
	; A slow flush may yield after taking its buffer snapshot. Re-check before the
	; fired row and every realtime sink so a pause transition cannot continue the
	; deferred pipeline under native Suspend.
	if A_IsSuspended && !Keylogger._shutting_down
		return false
	if IsSet(publish_guard) && !publish_guard.Call()
		return false
	...
	), &FireRejectedBySuspend)
	; Native Suspend may win after the pre-call generation check but before the
	; append's own guard. That is the sole retryable rejection: validation and
	; privacy drops are terminal and must not be replayed in a later app context.
	if FireRejectedBySuspend
		return false
	; A native-Suspend refusal remains queued for resume; an accepted row or a
	; terminal privacy/validation drop must never be duplicated in a later context.
	if IsSet(publish_guard) && !publish_guard.Call()
		return true
	; A privacy/validation drop consumes the queued fire but must not leak into
	; derived ROI or WPM stores after the canonical row was refused.
	if !FireAccepted
		return true
	Keylogger.last_flush_time := A_TickCount
	; On a suspended shutdown the durable rows above are terminalized under the
	; shutdown lease, but live ROI/WPM state must remain inert: no UI or session
	; accumulator is allowed to mutate while the user sees the driver as paused.
	if A_IsSuspended
		return true
