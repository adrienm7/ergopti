 ...
 * 3. Render Optimization: Utilizes requestAnimationFrame for fluid live-typing table updates.
 * 4. Absolute Completeness: Removes restrictive filters to show all raw unicode and symbols.
 * ==============================================================================
 */
	...
	if (Object.keys(window.metrics_manifest).length > 0) {
		...
		if (app_state.selected_apps.size === 0 || hadAllSelected) {
			...
			app_state.available_apps.forEach((app) => {
				if (app_state.selected_apps.has(app) || !previousApps.has(app)) nextSelection.add(app);
			});
...
function compute_manifest_metrics() {
	...
	manifest_dates.forEach((dateStr) => {
		...
		Object.keys(window.metrics_manifest[dateStr]).forEach((appName) => {
			...
			const eff_manual_wpm_chars = Math.max(0, filtered_manual_chars - pauses_raw);
			const effective_chars_for_speed =
				eff_manual_wpm_chars + filtered_hs_chars + filtered_llm_chars;
			const effective_chars_for_volume =
			...
			app_state.time_series[dateStr].chars += effective_chars_for_volume;
			app_state.time_series[dateStr].wpm_chars += effective_chars_for_speed;
	...
	sorted_keys.forEach((k) => {
		...
		let day_wpm = d.wpm_chars >= 10 && d.time_ms > 0 ? d.wpm_chars / 5 / (d.time_ms / 60000) : 0;
	...
	const manifest_cpm = global_time_total > 0 ? wpm_chars_total / (global_time_total / 60000) : 0;
...
function apply_local_filters() {
	...
	if (include_today && app_state.today_live_data) {
		Object.keys(app_state.today_live_data).forEach((appName) => {
			if (appName !== 'Unknown' && !app_state.available_apps.includes(appName)) {
				app_state.available_apps.push(appName);
				app_state.available_apps.sort((a, b) => a.localeCompare(b));
				app_state.selected_apps.add(appName);
				update_app_btn_text();
			}
...
function render_charts() {
	...
	sorted_keys.forEach((k) => {
		...
		let wpm = d.wpm_chars >= 10 && d.time_ms > 0 ? d.wpm_chars / 5 / (d.time_ms / 60000) : 0;
	...
	sorted_keys.forEach((k) => {
		...
		precision_pts.push({ x: new Date(k + 'T12:00:00'), y: accuracy });
	});
			...
			scales: {
				...
				y: {
					...
					ticks: {
						font: { size: 10 },
						maxTicksLimit: 3,
						callback: (val) => format_number(val) + '%'
					},
					grid: { color: 'rgba(128, 128, 128, 0.1)' }
				}
			},
...
function render_current_tab() {
	...
	Object.keys(source_dict).forEach((k) => {
		...
		let display_len = Array.from(norm_k).length || 1;

		// Absolutely no filtering based on sequence length or string patterns
		data_arr.push({
			key: k,
			count: item.count,
			...
			avg: isNaN(avg) ? 0 : avg,
			wpm:
				app_state.current_tab !== 'sc' && !isNaN(avg) && avg > 0
	...
	const pause_thresh = parseInt(document.getElementById('pause_threshold').value, 10) || 2000;

	let total_occ = 0;
	let total_time = 0;
	let valid_occ_for_speed = 0;

	app_state.rendered_list.forEach((i) => {
		total_occ += i.count;

		// If the sequence's average keystroke delay is under the UI threshold, we count it for the table's WPM
		if (i.avg <= pause_thresh) {
			total_time += i.count * i.avg;
			valid_occ_for_speed += i.count;
		}
	...
	let wpm_filtered = 0;
	if (total_time > 0) {
		cpm_filtered = valid_occ_for_speed / (total_time / 60000);
