...
| Ollama readiness     | synchronous `hs.execute(curl --max-time 5)` per attempt in `models_manager_ollama.lua:170-197`            | **Performance hypothesis**, not promoted without target profile                                           |
...
| `ui/menu/menu_llm/**`, MLX/Ollama managers                                           | `F HS-008`     | `F HS-007,009,010,018` | `F HS-007–010,018` | `S`                    | `F HS-018` | Two LLM dry passes; synchronous curl remains an unmeasured performance hypothesis.                                                                                                                                                                                         |
...
| `infra/vscode_bridge`                                                                | `B`            | `F HS-014`             | `B`                | `B/S`                  | n/a        | Bind contract verified against primary source; AX cache blanched.                                                                                                                                                                                                          |
...
| Menubar/WPM/UI builders                                                              | `B`            | `F HS-012,018`         | `F HS-012,018`     | `S/N profile`          | `F HS-018` | Correctness paths rechecked once; mouseMoved/canvas optimization requires profiles.                                                                                                                                                                                        |
...
- Synchronous `hs.execute`/`os.execute` calls in Ollama readiness, MLX adoption, input-source helpers, onboarding helpers, and backend menu actions can block the single Hammerspoon runloop. Without a production profile/reproduction on the target Mac, their latency impact remains a performance hypothesis.
