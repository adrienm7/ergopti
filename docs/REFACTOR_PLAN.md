...
- [ ] *(optionnel, cosmétique)* extraire `lib/locale.ahk` d'`i18n.ahk` (miroir de `macos/lib/locale.lua` : couche chargement-des-chaînes + `t()`). Déplacement ~250 l. dans le module le plus sensible à l'encodage, **zéro changement de comportement** — même classe « churn > valeur » que les moves cosmétiques déjà reportés (P2/P3). À faire seulement si la parité-miroir des noms de fichiers est jugée prioritaire.
