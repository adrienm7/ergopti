...
* **Symptom**: The macOS diagnostic window (`lib/healthcheck.lua`) showed `unknown`/`n/a` for almost every runtime field (LLM, layout, keylogger, hotstrings, log paths) and every boot/reload logged a wall of `[healthcheck] X is not a function` / `X unavailable` WARNINGs.
