...
local bare_defer = src:find("doAfter(0, M.toggle_right_click)", 1, true) ~= nil
helpers.assert_true(
	not bare_defer,
	"doAfter(0, M.toggle_right_click) must not appear bare — must be guarded with 'if rightClickHeld then' (gestures-actions-rclick)"
)

local guarded_defer = src:find("if rightClickHeld then M.toggle_right_click()", 1, true) ~= nil
helpers.assert_true(
	guarded_defer,
