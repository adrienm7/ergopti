...
--- 4. WebView Form: A single folder field with a "Parcourirâ€¦" button replaces
...
--- bootstrap files stay visually identical, and is intentionally English â€”
...
local function serialize_toml()
	...
	local lines = { "# Custom paths â€” auto-generated by ErgoptiPlus." }
...
local function load_bootstrap()
	...
	if not fh then
		Logger.info(LOG, "paths.toml not found â€” generating with defaults at '%s'.", paths_file())
		_bootstrap = {}
		save_bootstrap()
		return
	end
...
function M.init(base_dir, reload_fn)
	if type(base_dir) ~= "string" or base_dir == "" then
		Logger.error(LOG, "M.init(): base_dir must be a non-empty string â€” module non-functional.")
		return
	end
	_base_dir  = base_dir
...
--- right after this call and triggers its own reload at the end â€”
...
function M.persist_config_dir_for_wizard(new_dir)
	...
	-- An empty path or one equal to the default â†’ clear the override so
...
local function apply_and_reload(new_dir)
	...
	Logger.start(LOG, "Applying new config directory and reloadingâ€¦")
...
local function inject_init_data()
	...
	Logger.debug(LOG, "Injecting initData into webviewâ€¦")
	pcall(function()
...
local function handle_message(body)
	...
	elseif action == "browse" then
		hs.timer.doAfter(0, function()
			Logger.start(LOG, "Opening native folder pickerâ€¦")
			...
			if picked and picked ~= "" then
				...
				Logger.warn(LOG, "browse: picker returned nothing â€” user cancelled.")
			end
		end)
	...
	_webview = ui_builder.show_webview({
		...
		on_navigation = function(action)
			if action == "didFinishNavigation" then
				Logger.debug(LOG, "Navigation finished â€” injecting initData.")
...
--- Builds the "Dossier de configurationâ€¦" menu item for the tray menu.
