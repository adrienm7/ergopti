...
---
--- ROOT CAUSE ENCODED: KE v15.7 renamed karabiner_grabber to
--- Karabiner-Core-Service. The check previously only tested the legacy binary
--- path, so a machine with only the renamed service returned grabber_present=false, causing
...
--- The fix: check the legacy path, current path, AND karabiner_cli (stable across all
...
-- =========================================================================
-- =========================================================================
-- ======= 1/ is_grabber_binary_present: legacy and current coverage =======
-- =========================================================================
-- =========================================================================

helpers.describe("onboarding.is_grabber_binary_present: renamed-service detection", function()
	helpers.it("uses the v15.7+ app-bundle executable path", function()
	...
	helpers.it("returns true when only the legacy binary (karabiner_grabber) exists", function()
	...
	helpers.it("returns true when only the v15.7+ binary (Karabiner-Core-Service) exists", function()
		local mod = fresh_onboarding()
		with_io(function(path, _)
	...
	helpers.it("returns true when both legacy and current binaries exist", function()
		local mod = fresh_onboarding()
	...
	helpers.it("returns false when no KE binary exists (legacy, current, or cli)", function()
		local mod = fresh_onboarding()
