...
-- The gesture fires on finger-lift, which can generate a spurious leftMouseUp
-- that would immediately cancel selection mode.
local SELECTION_COOLDOWN_SEC = 0.15
	...
	mouseEventTap = hs.eventtap.new(
		...
		function(e)
			...
			if t == evTypes.leftMouseUp then
				...
				-- cancel selection mode before the user has a chance to drag.
				if hs.timer.secondsSinceEpoch() - activationTime < SELECTION_COOLDOWN_SEC then
					return true
				end
				-- Ignore leftMouseUp events generated by new trackpad contact when a
				-- new gesture is starting; the engine will re-trigger selection_toggle
			...
			-- In areas where the synthetic leftMouseDown was not accepted, the OS
			-- emits mouseMoved instead of leftMouseDragged.  Converting here gives
			-- a best-effort drag signal to apps that do honour leftMouseDragged
...
--- Used by the engine to prevent leftMouseUp events generated by new trackpad
--- contact from prematurely cancelling an active drag selection.
