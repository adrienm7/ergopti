// CODE FONDÉ EN MAJEURE PARTIE SUR LE SCRIPT ESPACEFINE DE VICTOR LOUX. AJOUT DU REMPLACEMENT DES APOSTROPHES EN APOSTROPHES TYPOGRAPHIQUES.
 ...
 * @return {void}
 */
function typography(searchNode) {
	/**
	...
	var regex1 = new RegExp(/(')/g),
		 ...
		 * @type {String}
		 */
		espace = '<span class="insecable" style="font-size: 0.67em">&nbsp;</span>',
		// espace = '&#x202F;',
		 ...
		 */
		balisesExclues =
			'html,head,style,title,link,meta,script,object,iframe,pre,kbd,code,textarea,noscript';
	...
	var _nodes,
		...
		_fragment,
		_isNodeList =
			typeof searchNode === 'object' &&
			/^\[object (HTMLCollection|NodeList|Object)\]$/.test(
				Object.prototype.toString.call(searchNode)
			) &&
			Object.prototype.hasOwnProperty.call(searchNode, 'length') /* || 'length' in searchNode */ &&
			(searchNode.length === 0 ||
				(typeof searchNode[0] === 'object' && searchNode[0].nodeType > 0)),
	...
	while (_nodeIndex--) {
		...
		if (
			_nodeActuel.nodeType === 1 &&
			_balisesExclues.indexOf(_nodeActuel.nodeName.toLowerCase()) === -1
		) {
			// Invoker la fonction par son nom plutôt qu'avec `arguments.callee`
			// pour éviter les erreurs ES5
			typography(_nodeActuel);
		}
		...
		// Si le node traversé n'est pas un node de texte,
		// ne rien faire et passer au node suivant
		if (_nodeActuel.nodeType !== 3) {
			continue;
		}

		// Sinon, remplacer l'espace normale & ajouter notre espace fine ET CHANGER L’APOSTROPHE
		_parent = _nodeActuel.parentNode;
		_fragment = (function () {
			var html1 = _nodeActuel.data.replace(regex1, '’');
			(html = html1.replace(regex2, '$1' + espace + '$2')),
