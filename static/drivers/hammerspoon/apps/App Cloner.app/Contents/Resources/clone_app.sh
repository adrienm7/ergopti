...
if [[ "$APP_FAMILY" == "teams" ]]; then
	...
	# Original executable name (e.g., MSTeams) — needed by the launcher to
	# exec the real binary after setting up the isolated environment.
	ORIG_EXE="$SRC_EXE_NAME"

	# Write the per-clone Info.plist override using PlistBuddy (atomic edits)
	...
	/usr/libexec/PlistBuddy -c "Set :CFBundleExecutable launcher" "$PLIST_PATH" 2>/dev/null \
		|| /usr/libexec/PlistBuddy -c "Add :CFBundleExecutable string launcher" "$PLIST_PATH"
	...
	# CFBundleIconFile (PNG/icns file) takes precedence.
	/usr/libexec/PlistBuddy -c "Delete :CFBundleIconName" "$PLIST_PATH" 2>/dev/null || true
	log "Info.plist patched: id=$UNIQUE_ID name=$safe_name exe=launcher"

	# Per-clone HOME — Teams stores everything under $HOME/Library/* so a
	# per-clone HOME yields fully isolated accounts, settings, caches.
	CLONE_HOME="$HOME/Library/Application Support/AppCloner/${safe_name}_home"
	mkdir -p "$CLONE_HOME/Library"

	# Symlink generic shared dirs so Teams behaves like a normal Mac app
	# (Downloads/Desktop accessible) rather than seeing an empty home.
	for d in Downloads Desktop Documents; do
		[[ ! -e "$CLONE_HOME/$d" ]] && ln -s "$HOME/$d" "$CLONE_HOME/$d" 2>/dev/null || true
	done
	log "CLONE_HOME=$CLONE_HOME"

	# Launcher: minimal — set HOME, exec the real Teams binary. We keep
	# the binary in place at MacOS/$ORIG_EXE; CFBundleExecutable now points
	# to "launcher" (next to it), so macOS launches us, and we exec the
	# real binary so Teams' window/process is the one Dock tracks.
	cat > "$MACOS/launcher" <<TEAMSLAUNCHER
#!/bin/zsh
# Generated by AppCloner (teams full-clone). Do not edit.
set -e
...
# Spawn the real Teams binary as our child so Dock keeps the launcher PID
# linked to the running window. Teams resolves \$HOME on startup → reads
# state from our isolated tree → fully separate account from the main
# Teams install.
"\$DIR/${ORIG_EXE}" &
APP_PID=\$!

if [[ -n "\$OPEN_ARG" && "\$OPEN_ARG" =~ \$URL_SCHEME_RE ]]; then
	# Wait for Teams to register its URL handler within our HOME-isolated
	# instance, then dispatch the URL by invoking the binary again with
	# the URL as positional arg (open -a would target the system-default
	# Teams, bypassing our clone).
	sleep 4
	"\$DIR/${ORIG_EXE}" "\$OPEN_ARG" >/dev/null 2>&1 || true
fi

wait "\$APP_PID"
TEAMSLAUNCHER
	chmod +x "$MACOS/launcher"
	log "Teams launcher written ($MACOS/launcher → exec $ORIG_EXE with HOME=$CLONE_HOME)"
