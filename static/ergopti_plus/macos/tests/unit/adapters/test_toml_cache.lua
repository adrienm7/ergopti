...
helpers.describe("adapters.toml_cache: snapshot round-trip + invalidation", function()
	...
	helpers.it("rejects a version-4 snapshot produced before duplicate definitions failed closed", function()
		cur_mtime, cur_size = 1000.5, 4242
		helpers.assert_eq(cache.store(SRC, { legacy_duplicate = true }, cache.capture_source(SRC)), true)
		local path = snapshot_path(SRC)
		local read_handle = assert(io.open(path, "r"), "cannot read current snapshot fixture")
		local payload = assert(read_handle:read("*a"))
		read_handle:close()
		local legacy_payload, replacements = payload:gsub("ver=5", "ver=4", 1)
		helpers.assert_true(replacements <= 1, "the snapshot must carry one schema version")
		local write_handle = assert(io.open(path, "w"), "cannot create version-4 snapshot fixture")
		write_handle:write(legacy_payload)
		write_handle:close()

		helpers.assert_nil(cache.load(SRC),
			"a cache generated by the duplicate-accepting parser must be invalidated")
	end)
