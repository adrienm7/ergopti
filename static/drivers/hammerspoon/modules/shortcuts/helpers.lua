...
function M.do_transform(transform_func)
	Logger.debug(LOG, "Starting text transformation…")
	...
	timer.doAfter(0.2, function()
		...
		if not sel or sel == "" then
			...
			Logger.warn(LOG, "No text selected for transformation.")
			return
		end
		...
		if not ok or not transformed then
			if prior then pcall(function() pasteboard.setContents(prior) end) end
			Logger.error(LOG, "Text transformation failed.")
			return
		end
		...
		timer.doAfter(0.08, function()
			...
			timer.doAfter(0.08, function()
				...
				timer.doAfter(0.15, function()
					...
					Logger.info(LOG, "Text transformation completed.")
				end)
			end)
		end)
	end)
...
function M.pixel_hex_at(x, y)
	Logger.debug(LOG, "Reading pixel color…")
	...
	if not ok_cap then
		Logger.error(LOG, "Screen capture for pixel read failed.")
		return nil
	end
...
"
	...
	if ok_py and out then
		...
		if hex then
			Logger.info(LOG, "Pixel color read successfully.")
			return hex
		end
	end
	
	Logger.warn(LOG, "Python pixel extraction returned no valid hex code.")
	return nil
end
...
function M.toggle_awake()
	if awake_active then
		...
		Logger.info(LOG, "Keep-awake module disabled.")
		...
		schedule_awake_tick()
		Logger.info(LOG, "Keep-awake module enabled.")
	end
end
...
function M.interactive_screenshot()
	Logger.debug(LOG, "Starting interactive screenshot process…")
	local ok, task = pcall(hs.task.new,
		...
		function(exit_code, _, _)
			if exit_code == 0 then
				...
				Logger.info(LOG, "Screenshot successful.")
			else
				Logger.warn(LOG, "Screenshot failed or cancelled.")
			end
		end,
		{ "-i", "-c" }
	)
