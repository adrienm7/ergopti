...
| **Defaults éparpillés**                  | `manifest.toml` couvre les _toggles_, mais des défauts non-toggle traînent : `windows/infra/llm_defaults.ahk`, `windows/infra/hotstrings/hotstrings_config.ahk`, `windows/infra/tap_hold/*`, et côté HS les tables `DEFAULT_STATE` dans `macos/modules/*/init.lua`               | même valeur définie en 2+ endroits → divergence silencieuse driver vs driver                                                                                                                         |
