...
<section>
	...
	<p>
		...
		>. C’est un logiciel très complet qui permet de modifier des dispositions de clavier sur
		Windows. Il est en mesure de créer des pilotes pour Windows, et depuis peu pour Mac. Seul Linux
		n’est pas supporté.
	</p>
	<!-- <p>
	...
	<p>
		Ce fichier se modifie avec un éditeur de texte afin que vous puissiez l’adapter selon vos
		envies, notamment pour en désactiver des fonctionnalités ou en ajouter. N’oubliez pas de le
		relancer pour appliquer vos modifications. Le raccourci <kbd>AltGr</kbd> + <kbd>BackSpace</kbd> a
		été implémenté afin de relancer facilement le script après une modification.
	</p>
	<div class="encadre">
		Il est possible d’utiliser Ergopti sans même installer de pilote. Pour cela, il suffit de passer
		à 1 la variable de la ligne 8 du fichier ErgoptiPlus.ahk. Idéalement, il vaut mieux utiliser le
		vrai pilote, notamment pour que le clavier soit Ergopti même sur l’écran de démarrage pour taper
		son mot de passe de session. Toutefois, la version sans pilote peut être utile <strong
			>pour tester Ergopti sans l’installer, ou sur des ordinateurs professionnels où l’on n’a pas
	...
	<ol>
		...
		<li>
			...
			exécuté au démarrage de Windows. Créer un raccourci dans ce dossier pointant vers
			l'emplacement où vous allez sauvegarder votre fichier <em>ErgoptiPlus.ahk</em>. Vous pouvez
	...
	<div>
		{#if version_mineure_ahk !== undefined}
			...
			<p>
				...
				script AutoHotkey compilé, il ne nécessite donc même pas d’installer AutoHotkey. Il suffit
				de le lancer pour que le clavier passe en Ergopti. Une icône bleue en forme d’étoile
