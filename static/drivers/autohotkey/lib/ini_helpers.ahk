...
ReadPathsToml(FilePath) {
    ...
    if !FileExist(FilePath) {
        ; Auto-generate with an example comment so users know what to set
        try {
    ...
    loop parse, FileRead(FilePath), "`n", "`r" {
        ...
        if RegExMatch(Line, '^(\S+)\s*=\s*"(.*)"$', &Match) {
            ; Normalise forward slashes to backslashes so users can write POSIX-style
            ; paths in the TOML file without triggering linter warnings about escape sequences
