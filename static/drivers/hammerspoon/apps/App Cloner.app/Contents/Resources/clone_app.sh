...
if [[ "$PWA_MODE" == "1" ]]; then
	...
	# Stored as a file in the profile dir so the launcher can call it without
	# embedding a nested heredoc (zsh does not support heredocs inside heredocs).
	mkdir -p "${PWA_PROFILE_DIR}"
...
SWIFTEOF
	...
	# The PWA launcher: stays alive as the stub's main process so the Dock tile
	# runs under our bundle-id (not Edge's). Edge is a child we wait on.
	cat > "$LAUNCHER" << PWAEOF
#!/bin/zsh
...
SWIFT_HELPER="${PWA_PROFILE_DIR}/switch_space.swift"
...
if [[ -f "\$LOCK_FILE" ]]; then
	...
	if [[ -n "\$OLD_PID" ]] && kill -0 "\$OLD_PID" 2>/dev/null; then
		# Launcher still alive — bring the Edge child window to the front.
	...
	# Stale lock — previous launcher crashed. Remove it and start fresh.
	rm -f "\$LOCK_FILE"
fi

# Write our PID so a second Dock click can focus us instead of opening a new window.
echo \$\$ > "\$LOCK_FILE"
trap 'rm -f "\$LOCK_FILE"' EXIT INT TERM

# On first launch, switch to the target Space so Edge opens there naturally.
# We consume the hint file once; subsequent launches let Edge remember its own position.
if [[ -f "\$SPACE_HINT" ]]; then
	TARGET_UUID="\$(cat "\$SPACE_HINT")"
	rm -f "\$SPACE_HINT"
	[[ -f "\$SWIFT_HELPER" ]] && swift "\$SWIFT_HELPER" "\$TARGET_UUID" 2>/dev/null || true
fi

# Launch Edge as a child (not exec) so this launcher stays alive and the Dock
