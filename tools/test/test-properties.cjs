...
function prop(label, arb, predicate, opts = {}) {
	try {
		fc.assert(fc.property(arb, predicate), { numRuns: RUN_COUNT, ...opts });
		total_pass++;
...
function makeRegistry() {
	...
	return {
		...
		add(trigger, repl, opts) {
			...
			bucket.sort((a, b) => b.tlen - a.tlen || a.group_order - b.group_order || a.seq - b.seq);
			return mapping;
		},
...
const arbMappingOpts = fc.record({
	...
	color: fc.oneof(
		fc.constant(null),
		fc.stringMatching(/^[0-9a-f]{6}$/).map((h) => `#${h}`)
	)
});
...
const arbMappingList = fc
	.array(arbMapping, { minLength: 1, maxLength: 20 })
	.map((list) => {
		const seen = new Set();
...
suite('HotstringMatcher — determinism', () => {
	...
	prop(
		...
		(mappings) => {
			...
			for (const m of registry._all()) {
				const result = matchBuffer(m.trigger, m.tail_char, registry, {
...
suite('HotstringMatcher — empty / boundary cases', () => {
	prop('no match on empty buffer — regardless of registry content', arbMappingList, (mappings) => {
		const registry = makeRegistry();
		for (const { trigger, repl, opts } of mappings) {
			registry.add(trigger, repl, opts);
		}
	...
	prop('no match when registry is empty — regardless of buffer', arbBuffer, (buffer) => {
		const registry = makeRegistry();
		const tail = [...buffer].at(-1) ?? 'x';
...
suite('Registry — invariants', () => {
	prop('validateAdapter passes for a freshly built in-memory registry', fc.constant(null), () => {
		const registry = makeRegistry();
	...
	prop('size() equals the total number of added entries', arbMappingList, (mappings) => {
		const registry = makeRegistry();
		for (const { trigger, repl, opts } of mappings) {
			registry.add(trigger, repl, opts);
		}
	...
	prop('clear() results in size() == 0', arbMappingList, (mappings) => {
		const registry = makeRegistry();
		for (const { trigger, repl, opts } of mappings) {
			registry.add(trigger, repl, opts);
		}
	...
	prop('tlen matches codepoint length of trigger', arbAsciiTrigger, (trigger) => {
		const registry = makeRegistry();
