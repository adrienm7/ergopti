...
function M.start()
	...
	Logger.debug(LOG, "Starting shortcuts bindings…")
	started = true
	...
	Logger.info(LOG, "Shortcuts bindings started.")
end

--- Unbinds all hotkeys and stops background tasks.
function M.stop()
	if not started then return end
	Logger.debug(LOG, "Stopping shortcuts bindings…")
	...
	hotkeys = {}
	started = false
	Logger.info(LOG, "Shortcuts bindings stopped.")
end
...
function M.enable(name)
	...
	if type(def) == "function" then 
		...
		if ok and type(obj) == "table" then
			hotkeys[name] = obj
			Logger.info(LOG, string.format("Hotkey %s enabled.", name))
		end
	end
end
...
function M.disable(name)
	...
	hotkeys[name] = nil
	Logger.info(LOG, string.format("Hotkey %s disabled.", name))
end
