...
_TLDHCL_HasCorrectionsAfterLTrim() {
	...
	Assert(InStr(Src, "LTrim(chunks[1].text)") > 0,
		"infra/llm_diff.ahk must LTrim chunks[1].text to strip leading whitespace from the first equal chunk")

	; HasCorrections assignment must be present
	Assert(InStr(Src, "has_corrections") > 0,
		"infra/llm_diff.ahk must compute a has_corrections flag")
	...
	Assert(InStr(Src, "chunks[1].text != " . Chr(0x22) . Chr(0x22)) > 0,
		'infra/llm_diff.ahk must guard HasCorrections with chunks[1].text != "" to exclude whitespace-only corrections after LTrim')
	...
	Assert(LtrimPos > 0 and HcorrPos > 0,
		"Both LTrim(chunks[1].text) and has_corrections := must be present in infra/llm_diff.ahk")
	Assert(LtrimPos < HcorrPos,
		"infra/llm_diff.ahk must evaluate has_corrections AFTER the LTrim pass on chunks[1].text (ordering matters)")
}
