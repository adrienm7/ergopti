...
local _start_ts  = nil
local _ready     = false
local _queued    = {}   -- Events received before the DOM was ready
...
function M.show(model_name, on_cancel)
    ...
    local W, H = 380, 240
    local x = f.x + f.w - W - 18
    local y = f.y + f.h - H - 50

    local ok, wv = pcall(hs.webview.new, {x=x, y=y, w=W, h=H}, {}, _ucc)
    if not ok or not wv then
        print("[download_window] Error creating webview.")
        return
    end
    
    _wv = wv
    pcall(function() _wv:windowStyle({"titled", "closable", "nonactivating"}) end)
    pcall(function() _wv:windowTitle("Téléchargement du modèle") end)
    pcall(function() _wv:level(hs.drawing.windowLevels.floating) end)
    pcall(function() _wv:allowGestures(false) end)
    pcall(function() _wv:allowNewWindows(false) end)
    ...
    pcall(function() _wv:html(final_html) end)
    pcall(function() _wv:show() end)
