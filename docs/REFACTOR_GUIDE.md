...
| ✅ | **P9.2** | « un déplacement ne casse aucun test » · 136 file-pinnés | ✅ `test-git-mv-resilience.cjs` (JS gate) : scanne tous les `test_*.lua` macOS ayant un `driver_root() .. "…/x.lua"`, résout le chemin réel sur disque, et échoue si l'un est introuvable — détecte immédiatement tout `git mv` sans mise à jour du test correspondant. Complémentaire au ratchet P9.1 (empêche d'en ajouter) : P9.2 empêche qu'ils pourrissent. Câblé dans `test:js` (38 checks). Ratchet P9.1 baseline ajusté 133 → 136 (3 nouveaux invariants source délibérés de l'audit 2026-06-30, chacun doublé d'une assertion comportementale dans la section suivante). |
