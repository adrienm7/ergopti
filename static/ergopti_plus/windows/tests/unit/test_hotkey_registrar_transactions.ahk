...
; DESCRIPTION:
; Behavioural proof that every dynamic hotkey has one physical owner, that an
...
_HKRT_Reset() {
	...
	global _HKRT_ProbeThrows, _HKRT_ReentrantHandle
...
_HKRT_Probe(Name) {
	global _HKRT_NativeBindings, _HKRT_ProbeThrows, _HKRT_BeforeProbeHook
...
_HKRT_ReserveFailureRestoresPriorTombstone() {
	...
	PriorEntry := HOTKEY_REGISTRAR_SPECS["^j"]
	_HKRT_ThrowInstallSpecs["^j"] := true
	AssertEqual("", _HKRT_Reserve("Ctrl+J", _HKRT_Second, "metrics:apps"))
	AssertTrue(HOTKEY_REGISTRAR_SPECS["^j"] == PriorEntry,
...
_HKRT_ActivateFailureRestoresExactReserve() {
	...
	_HKRT_Events := []
	_HKRT_FireBeforeActions["^t On"] := true
	_HKRT_ThrowOnSpecs["^t"] := true
	AssertFalse(_HotkeyRegistrarActivate(Handle, _HKRT_NativeHotkey))
	_HKRT_AssertEvents(["^t On"],
...
_HKRT_PublicBindFailureHasNoGhostAction() {
	...
	_HKRT_Reset()
	_HKRT_FireBeforeActions["^q On"] := true
	_HKRT_ThrowOnSpecs["^q"] := true
	AssertEqual("", _HKRT_Bind("Ctrl+Q", _HKRT_First, "metrics:typing"))
	_HKRT_AssertEvents(["^q Off", "^q On"],
	...
	AssertEqual(1, _HKRT_AppDeliveries)
	AssertEqual(0, HotkeyRegistrarLiveCount())
	AssertEqual("retired", HOTKEY_REGISTRAR_SPECS["^q"]["state"]["phase"])
	_HKRT_PhysicalPress("^q")
...
_HKRT_EnableFailureRestoresExactDisabledState() {
	...
	Before := HOTKEY_REGISTRAR_BINDINGS[Handle]["state"]
	_HKRT_Events := []
	_HKRT_FireBeforeActions["^e On"] := true
	_HKRT_ThrowOnSpecs["^e"] := true
	AssertFalse(_HotkeyRegistrarSetEnabled(Handle, true, _HKRT_NativeHotkey))
	_HKRT_AssertEvents(["^e On"],
	...
	AssertEqual(1, _HKRT_AppDeliveries)
	_HKRT_ThrowOnSpecs.Delete("^e")
	AssertTrue(_HotkeyRegistrarSetEnabled(Handle, true, _HKRT_NativeHotkey))
	_HKRT_PhysicalPress("^e")
	AssertEqual(1, _HKRT_FirstCalls)
}
...
_HKRT_OwnersCannotBeClobbered() {
	...
	_HKRT_PhysicalPress("^b")
	AssertEqual(1, _HKRT_FirstCalls)
	AssertEqual(0, _HKRT_SecondCalls)

	_HKRT_NativeBindings["^q"] := { callback: _HKRT_First, enabled: true }
...
_HKRT_PhysicalAliasesHaveOneOwner() {
	global _HKRT_FirstCalls
	_HKRT_Reset()
	AssertEqual("^!+#f9",
	...
	AssertEqual("", _HKRT_Bind("Ctrl+Escape", _HKRT_Second, "llm:trigger"),
		"physical aliases must not create sibling owners")
	_HKRT_PhysicalPress("^escape")
	AssertEqual(1, _HKRT_FirstCalls)
}
Test("hotkey registrar: physical aliases and modifier order share one owner "
	. "(hotkey-physical-alias-single-owner)", _HKRT_PhysicalAliasesHaveOneOwner)
