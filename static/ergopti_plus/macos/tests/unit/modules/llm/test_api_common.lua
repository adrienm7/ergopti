...
helpers.describe("ApiCommon.get_stop_sequences: stop-sequences single source (D-3)", function()
	helpers.it("returns a non-empty table for batch", function()
		local seq = ApiCommon.get_stop_sequences("batch")
		helpers.assert_true(type(seq) == "table" and #seq > 0, "batch must be a non-empty table")
	end)

	helpers.it("line is strictly longer than batch", function()
		local batch = ApiCommon.get_stop_sequences("batch")
		local line  = ApiCommon.get_stop_sequences("line")
		helpers.assert_true(#line > #batch, "line must extend batch with newline/separator stops")
	end)

	helpers.it("first element of every unified variant is <|eot_id|>", function()
		for _, v in ipairs({ "batch", "line" }) do
			local seq = ApiCommon.get_stop_sequences(v)
			helpers.assert_eq(seq[1], "<|eot_id|>", v .. " first token must be <|eot_id|>")
		end
	end)

	helpers.it("batch contains TAIL:", function()
		local seq = ApiCommon.get_stop_sequences("batch")
		local found = false
		for _, tok in ipairs(seq) do if tok == "TAIL:" then found = true ; break end end
		helpers.assert_true(found, "batch must contain TAIL:")
	end)

	helpers.it("line contains TAIL: (unified — MLX now uses the same superset as Ollama)", function()
		local seq = ApiCommon.get_stop_sequences("line")
		local found = false
		for _, tok in ipairs(seq) do if tok == "TAIL:" then found = true ; break end end
		helpers.assert_true(found, "line must contain TAIL: — all backends share the unified superset")
	end)
end)
