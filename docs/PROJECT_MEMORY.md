...
_Qui possede le terminateur, et pourquoi une expansion doit etre atomique — les defauts de frappe observes en 2026 et leur portee par driver_
...
**5. Certains terminaux rendent une rafale de suppressions depuis un etat perime.** Le journal AHK du 2026-08-20 prouvait que le matcher calculait correctement `{BackSpace 8}{Text}XGBoost ` et `{BackSpace 4}{Text}attention`, alors que le terminal affichait `xgboostXGBoost` et `att★attention`. Dans les TUI React concernes, les Backspace rapproches peuvent tous recalculer leur resultat depuis la meme valeur rendue, tandis que le texte suivant utilise une reference synchronisee et se concatene donc au declencheur intact. Les deux drivers classifient desormais les hotes terminal/TUI et partagent `[debounce].terminal_hotstring_key_delay_ms` : Windows conserve **une seule** rafale `SendEvent` sous `Critical` + `BlockInput("Send")`, et macOS poste la transaction taguee vers l'application exacte avec une pause entre les paires de suppression, tant que le callback d'origine retient encore les frappes physiques. **Ne jamais appliquer ce rythme aux applications graphiques, retirer les tags macOS, ni rescinder la transaction Windows.** Gardes : `windows/tests/meta/test_expansion_burst_atomic.ahk`, `macos/tests/unit/modules/keymap/test_expander.lua`, `macos/tests/unit/adapters/test_synthetic_input_provenance.lua`.
...
Abrupt death is fenced without stock-process control. The signed `ErgoptiPlus` executable has an outer headless role that owns Hammerspoon's pipes and wait-supervises one exact inner child over a private socket. The inner exclusively creates, signals, and reaps its own transient **lease-authority** `karabiner_cli` children; separate Hammerspoon runtime writers carry only exact generation-scoped non-authority names. An independent per-user `launchd` guardian now runs the same executable with `--karabiner-lease-guardian`. Before a generation may activate, its outer publishes and locks a durable exact-token record and waits for an ARMED acknowledgement from that guardian generation. On Hammerspoon EOF, outer loss, inner loss, guardian restart, STOP, or bounded CLI timeout, a surviving private role repeatedly writes only `ergopti_mode_<token>=0` plus the monotone `ergopti_revoked_<token>=1` tombstone. If Hammerspoon, outer, and inner all die together, release of the record lock wakes the `launchd` guardian; `KeepAlive` restarts the guardian itself after its own abrupt death. Record data cannot select an executable or arbitrary variable: the guardian derives both exact variable names from the canonical 32-hex filename and invokes only the canonical `karabiner_cli --set-variables` endpoint. It may signal and reap only direct CLI children it created, never enumerate or signal a stock Karabiner PID. The `script_quit` path calls `karabiner.shutdown("script_quit")` before `os.exit()` because that API bypasses the shutdown callback. A heartbeat reasserts the desired exact mode when the shared Core Service restarts and loses in-memory variables.

The guardian retries an exact fence until two successful transports span the confirmation grace; if the canonical `karabiner_cli` is unavailable, keyboard release can therefore be delayed until that endpoint recovers, but this never grants authority to kill or restart a shared Karabiner component. Real-Mac validation remains required for physical Activity Monitor Force Quit of the built app and private roles, Login Items registration/approval on supported macOS versions, Core Service restart, APFS lock/vnode behavior, and observable physical-keyboard release. Windows can exercise the Lua protocol, packaging contract, and deterministic models, but cannot compile or execute these native lifetime checks.
