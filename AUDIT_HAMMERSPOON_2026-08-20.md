...
- [x] `HS-003` — exact timer-owned Terminal serializer, global input fence, drain, and retry ownership — fixed and behaviorally replayed in this commit
...
**Implemented and behaviorally replayed.** Independent implementation lenses reopened eleven material blockers in the first candidate; the final production path and faithful native refusal/reordering doubles close each one:

1. [x] The keylogger bypasses provenance classification for mouse/scroll events. A tagged physical replay can therefore reclaim itself while it is still pending, loop forever, and lose the click. Route every taggable event through `EventProvenance.classify_with_fence()` and drive the real keylogger callback.
2. [x] `when_idle()` ignores already accepted post-eventtap callbacks. Pause can acquire its admission fence before a queued tooltip acceptance opens its transaction, consuming Tab/Enter and then rejecting `begin()`. Even after counting that queue, the accepted idle callback can recheck non-idle and recursively call `when_idle()` without consuming a new false/throw timer-acquisition result; the original caller already received `true`, so its callback is lost forever. The inverse is also reachable: when initial lifecycle dispatch and fallback all refuse, `when_idle()` returns `false` but leaves its non-post-callback entry queued, so a later unrelated enqueue can execute a pause transition already reported as rejected. Idle, recheck, refusal removal, and admission acquisition must share exact callback ownership and rearm settlement.
3. [x] Pause can orphan its exact admission fence when `get_enabled()` is absent/throws, when the no-integration path exits, when release refuses, or when stop interrupts a transition. Retain explicit release debt and never publish RESUMED until the same fence settles.
4. [x] Periodic paced/physical/idle timers are erased after an unchecked `stop()`. A native false/nil/throw leaves a self-retained 100–200 Hz timer with no retry owner; constructor results such as literal `false` are also indexed outside protection. Retain and autonomously retry the exact handle until stop is proven.
5. [x] A paced owner has both a recurring tick and a zero-delay wake. If the originating callback outlives one cadence, both can pump back-to-back and collapse the required Terminal spacing. A distinct reserved/physical owner's 5 ms periodic tick can also call the global FIFO pump and start a paced 20 ms head early, producing a 15 ms interval even when the paced owner's own wake is correct. Use one temporal authority or a monotonic per-head `next_due_at` gate and test cross-owner callback orders.
6. [x] A refused/throwing pre-teardown input drain is routed to `fail_after_teardown()` even though teardown never ran. The same post-teardown path is still used if `mark_reload` or logger execution fails immediately after the fence; existing tests explicitly expect `teardowns == 0`. Quit/reload can therefore fatal-exit with taps and resources still live. Every pre-teardown failure sequence must execute teardown before its fatal backstop.
7. [x] The new `clipboard_restore` producer is missing from the exhaustive synthetic reset inventory. Extend both the inventory and a behavioral drain test proving that pause/reload remain pending until all-type restoration settles exactly.
8. [x] The terminator watchdog destroys its sole timer when it fires before a long/retrying paced transaction becomes terminal, then does not rearm. If the later completion callback also loses its lifecycle wake, the reserved Enter and global drain remain pending forever. Keep a bounded liveness owner until either the transaction settles or the exact terminator is canceled, and test watchdog-before-completion plus dispatcher/fallback refusal.
9. [x] Multi-paste sequencing arms the intermediate key and following paste as independent timers with the same deadline, relying on insertion order that Hammerspoon does not guarantee. A refused later payload also releases its retain without failing the replacement transaction, so truncated output can publish `complete`. Give the sequence one ordered owner and make every deferred emission refusal fail the exact transaction; test reversed timer delivery and second-payload refusal.
10. [x] The multi-paste cursor can create deferred predecessor batches after the terminator has reserved its FIFO ordinal. `pop_deferred_batch()` permits those exact siblings to overtake the not-ready reservation, but both the generic unowned starter and the reserved periodic wake assume the literal head is dispatchable; one path stalls, while the other pops the predecessor sibling and asserts that it owns the reserved batch. Exercise Terminal pacing plus multi-paste plus Enter as one integrated transaction and preserve replacement/paste/terminator order without a blocked drain.
11. [x] If pause preflight fails and exact admission-fence release returns false or throws, the live token is retained but the next explicit pause request attempts a new fence instead of reusing or settling it. Acquisition therefore refuses forever while `_is_paused` still says false; only `stop()` retries. The in-service retry must reuse the exact retained owner, then either commit PAUSED or release it exactly, with no restart required.

**Implementation evidence.** Terminal callback output is now prepared, sealed, authorized, and transferred to a single process-wide FIFO without any native post or sleep inside the eventtap. Exact recurring owners retain paced, physical, reserved-terminator, idle, and cleanup authority; generic broker turns cannot borrow their cadence, and only exact predecessor siblings can cross a not-ready reservation. Physical replay remains globally routed, clipboard restoration participates in the same drain, and pause/reload/quit wait for every accepted producer before teardown. A broad 21-module pass reached `305/305` before the final cross-owner edge review; after the last fixes, the changed provenance, integrated terminator/multi-paste, and pause modules replayed at `74/74`, `4/4`, and `16/16`, while five direct serializer/terminator/clipboard/teardown neighbors passed `68/68`. All `259` production Lua sources compile; Hammerspoon integrity (`14/14`), LuaJIT 5.1 compatibility, closure-before-local, conventions, the zero-debt false-green ratchet, and `git diff --check` are green. No full-suite or native-macOS timing claim is made; Quartz target routing, native timer coalescence, and pasteboard behavior still require target smoke coverage.

**E2E harness follow-up.** The virtual-keyboard harness still expected a reserved terminator from a second callback collector, although the fixed serializer deliberately activates that successor without posting it on the caller's stack. That stale fixture produced the historical `60/64` snapshot. It now observes the real global `event:post(app)` boundary, requires zero terminator pairs in callback output, exactly one tagged down/up pair from the owned FIFO, and a bounded drain to zero active transactions. The complete harness is green at `67/67` with one corpus-declared driver-specific skip; no provenance assertion was removed or weakened.

Two narrower subcases were already closed in the first candidate—failed terminator completion registration cancels its exact reservation, and a missing replacement transaction no longer schedules a duplicate Enter. The final replay adds the previously missing real non-keyboard keylogger path and stateful recurring-timer refusal shapes, so those earlier fixture blind spots no longer carry the implementation claim.
...
**Implementation-phase siblings (same owner).** Editing a profile shortcut is also release-first and non-transactional. The function forgets the old handle before it knows whether `hs.hotkey.new()` acquired and enabled the replacement; construction refusal clears the live-handle slot, while `activate_hotkey()` discards an `enable()` throw and reports success. The shared registrar also treated `pcall` success as activation success even though native `enable()` may return `nil` when Hammerspoon refuses the chord. Both paths can persist a candidate chord that cannot fire after the prior chord has already been lost. The primary LLM trigger shortcut repeats the same sequence and additionally leaves candidate runtime/preferences published if persistence refuses after acquisition. Its shared prompt helper also discards the exact `on_apply` result and returns `true`, allowing the menu to announce success after the transaction refused. A preference-equality fast path can likewise treat `disabled` as a no-op while an exact native handle is still live, so the supposedly removed trigger continues firing. The `HS-033` hotkey transaction must cover fresh bind, activation, action-to-action rebind, disable despite preference equality, rollback, prompt-result propagation, and profile deletion for both trigger families, retaining the exact old owner and preference until the successor commits.
...
**Regression test.** Add `tests/unit/ui/menu/menu_llm/test_profile_delete_transaction.lua`. Use a native-shaped hotkey whose void `delete()` throws and a registrar seam whose release returns `false`/`nil`, invoke the real confirmed Delete action, and assert the old profile, shortcut preference, runtime registry, durable state, menu and exact handle remain. After the same handle settles, assert `set_user_profiles(kept)` commits once and the chord cannot execute `P`. Table-drive fresh bind, native `enable()==nil`, action-to-action rebind, disable, and persistence refusal for both primary and profile shortcuts: the exact old handle/chord and preference must remain, with no false menu success.

**Implemented and behaviorally replayed.** Primary and per-profile shortcuts now use the shared registrar behind closed callback gates; candidate acquisition, native activation, persistence, menu publication, predecessor release, rollback and cleanup debt are ordered under one exact owner. Native `enable()==nil` is a refusal, the opaque handle remains retryable, and both same-chord silent restoration and preference-equal disable still settle the live owner. Confirmed profile Delete acquires a deferred shortcut lease, publishes replacement registries only through `set_user_profiles()`, defers explicit-profile intent until the parent commits, and retains inverse compensation across runtime, persistence, menu or native refusal. Delete debt gates sibling selections and recommendations (including Cancel/no-op), while Delete first settles any older ModelSwitcher debt. Prompt callbacks propagate literal success instead of manufacturing it.

With production sources temporarily removed while retaining the new tests, the exact HS-033 module exited `1` with `115` failures out of `116`; the direct registrar module was red `20/21`, and removing only the real factory wiring made its integration module red `12/13`. Restored sources are green at `116/116`, `21/21`, and `13/13`; the changed modules plus direct transactional neighbours total `303/303`. The factory regression loads the real `MenuLLM.create`, proves the Delete gate is dynamically forwarded, and pins the exact ModelSwitcher settlement callback supplied to ProfilesManager. Independent second-lens review returned GO. These are deterministic Lua harnesses; no native macOS smoke result is claimed.
...
| -------------------- | --------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Terminal replacement | `D` timer-owned delete-pair turns at the configured `20 ms` cadence; zero payload `post`/`usleep` inside the eventtap | `HS-003` is fixed in the shim with exact timer/drain/provenance ownership; target latency remains unmeasured |
...
| `project-hs-native-eventtap-disable-recovery`             | Terminal prevention fixed; persistence work remains        | `HS-003` moves Terminal pacing fully out of the tap with exact ownership; `HS-004` still puts persistence work there, so recovery is not a substitute for finishing prevention.                                                                                                                                                     |
...
| `project-macos-eventtap-no-blocking`                      | Terminal path fixed; keylogger persistence remains         | `HS-003` removes inline Terminal pacing and owns its drain; `HS-004` remains.                                                                                                                                                                                                                                                        |
| `project-typing-order-and-atomicity` versus eventtap rule | Fix in place in the shim                                   | Exact-app replacement pacing, physical replay, pause admission, terminator order, and teardown drain are transactionally owned; native macOS smoke remains outstanding.                                                                                                                                                             |
...
| `modules/keymap/expander`, `adapters/{synthetic_input,event_provenance,text_sender}` | `B/F`          | `F HS-003,005`         | `F HS-003,005`     | `F HS-003`             | `F HS-005` | `HS-003` is behaviorally replayed through exact timer settlement, deferred-work drain, physical provenance, pause admission, terminator/clipboard ordering, and teardown; `HS-005` remains historical audit scope.                                                       |
...
1. `HS-003` is fixed; finish `HS-004`'s remaining keylogger eventtap work before another correctness change increases callback duration. `HS-025` is already implemented in the status register.
