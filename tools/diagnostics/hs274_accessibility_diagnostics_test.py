...
class DiagnosticTests(unittest.TestCase):
    ...
    def test_retains_identity_and_independent_errors_with_bounded_output(self):
        with TemporaryDirectory() as root:
            ...
            self.assertEqual(evidence["owner_pids"], [124])
            self.assertEqual(evidence["tcc_owner"]["stdout"], "msgID=124.2 denied")
            self.assertIn('eventMessage CONTAINS "msgID=124."', run.call_args.args[0][-1])
