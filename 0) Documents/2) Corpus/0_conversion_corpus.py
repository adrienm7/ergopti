chemin = "D:/Documents/Github/hypertexte/documents/Corpus/"

textes = ["500 Common Words in Prose", 
"Chained English Bigrams 7", 
"Common SAT Words",
"Corpus-nemolivier-propre",
"Essais",
"Harry Potter 1-8",
"La Farude",
"LdS",
...
def autocorrection(texte):
	...
	texte = texte.replace('siècle', 'sièc')
	return texte
...
def circonflexe_automatique(texte):
	texte = texte.replace('^a', 'êa')
	texte = texte.replace('â', 'êa')
	texte = texte.replace('^i', 'êi')
	texte = texte.replace('î', 'êi')
	texte = texte.replace('^e', 'ê')
	# texte = texte.replace('ê', 'e')
	texte = texte.replace('^o', 'êé')
	texte = texte.replace('ô', 'êé')
...
# Roulements sur la main gauche
def roulements_main_gauche(texte):
	# texte = texte.replace('ié', 'iê')
	# texte = texte.replace('éi', 'êi')
	return texte
...
def roulements_main_droite(texte):
	...
	texte = texte.replace('pt', "p'")
	texte = texte.replace('gt', 'gx')
	# texte = texte.replace("n't", "nt'")
	# texte = texte.replace('sch', 'shg')
	# texte = texte.replace('scr', 'shx')
	return texte




# Remplacements avec À
def raccourcis_avec_a(texte):
	texte = texte.replace('fs', 'àb')
	texte = texte.replace('sf', 'àf')
	texte = texte.replace('ds', 'àd')
	texte = texte.replace('cl', 'àl')
	texte = texte.replace('xp', 'àp')
	texte = texte.replace('j’', "à'")

	texte = texte.replace('u,', 'à,')
	texte = texte.replace('adm', 'àm')
	texte = texte.replace('cs', 'às')
	texte = texte.replace('nl', 'àn')
	texte = texte.replace('mp', 'àt')
	texte = texte.replace('marq', 'màr')
	texte = texte.replace('rq', 'àr')
	texte = texte.replace("sq", 'àq')

	texte = texte.replace('adv', 'àv')
	texte = texte.replace('cd', 'àc')
	texte = texte.replace('ph', 'àh')
	texte = texte.replace('règle', 'règ')
	texte = texte.replace('gl', 'àg') # Attention, car règle fait alors un SFB
	# texte = texte.replace('', 'àx')

	texte = texte.replace('aà', ',à') # À mettre à la fin pour éviter les SFB aà
...
def a_devient_j(texte):
	texte = texte.replace('aj', 'ày')
	texte = texte.replace('ja', 'àw')
	texte = texte.replace('ji', 'ài')
	texte = texte.replace('je', 'àe')
	texte = texte.replace('jo', 'ào')
	texte = texte.replace('ju', 'àu')
	texte = texte.replace('jé', 'àé')
	texte = texte.replace('j’', "à'")
	return texte



# Remplacements avec È
def raccourcis_avec_e(texte):
	texte = texte.replace('eo', 'eè')
	texte = texte.replace('oe', 'èe')
	texte = texte.replace('bu', 'èé')
	texte = texte.replace('ub', 'éè')
	return texte
...
def conversion(nom_fichier):
	input = open(chemin + "inputs/" + nom_fichier + ".txt", "r", encoding="utf-8")
	...
	nouveau_texte = roulements_main_gauche(nouveau_texte)
	...
	nouveau_texte = raccourcis_avec_a(nouveau_texte)
	nouveau_texte = a_devient_j(nouveau_texte)
	nouveau_texte = raccourcis_avec_e(nouveau_texte)
	nouveau_texte = circonflexe_automatique(nouveau_texte)
	nouveau_texte = touche_repetition(nouveau_texte)

	output = open(chemin + nom_fichier + " (converti).txt", "w", encoding="utf-8")
	output.write(nouveau_texte)
	output.close()



def conversion_liste(liste_fichiers):
	for nom_fichier in liste_fichiers:
		conversion(nom_fichier)



conversion("LdS")
# conversion_liste(textes)