...
- [ ] `modules/gestures.ahk` (2076 l.) + `lib/healthcheck.ahk` → sous-fichiers. ⚠️ **gestures = risque élevé** : hotkeys top-level `^#+F1::` dépendants de `#InputLevel 2` (non exercés par la suite headless → régression visible seulement au reload utilisateur) + ~18 tests d'introspection dépendants. À traiter avec un reload-test explicite.
