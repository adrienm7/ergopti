...
- **Source-file encoding rule:** all source files, including `.ahk`, MUST use LF line endings. AHK source files MUST additionally retain a UTF-8 BOM. Mixed line endings can make parser failures difficult to diagnose, so run `npm run test:ahk-encoding` after editing `.ahk` files. Do not use `cat >>` or a tool that changes the encoding; non-ASCII source is allowed only with the required BOM intact.
